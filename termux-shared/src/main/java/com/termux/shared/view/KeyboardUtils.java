...
public class KeyboardUtils {
    ...
    public static void setSoftKeyboardVisibility(
            @NonNull final Runnable showSoftKeyboardRunnable,
    ...
    /**
     * Toggle the soft keyboard. The {@link InputMethodManager#SHOW_FORCED} is passed as {@code
     * showFlags} so that keyboard is forcefully shown if it needs to be enabled.
     *
     * <p>This is also important for soft keyboard to be shown when a hardware keyboard is
     * connected, and user has disabled the {@code Show on-screen keyboard while hardware keyboard
     * is connected} toggle in Android "Language and Input" settings but the current soft keyboard
     * app overrides the default implementation of {@link
     * InputMethodService#onEvaluateInputViewShown()} and returns {@code true}.
     */
     ...
     * forcefully shown.
     *
     * <p>This is also important for soft keyboard to be shown on app startup when a hardware
     * keyboard is connected, and user has disabled the {@code Show on-screen keyboard while
     * hardware keyboard is connected} toggle in Android "Language and Input" settings but the
     * current soft keyboard app overrides the default implementation of {@link
     * InputMethodService#onEvaluateInputViewShown()} and returns {@code true}.
     ...
     *
     * <p>Also check {@link InputMethodService#onShowInputRequested(int, boolean)} which must return
     * {@code true}, which can be done by failing its {@code ((flags&InputMethod.SHOW_EXPLICIT) ==
     * 0)} check by passing {@code 0} as {@code flags}.
    ...
    /**
     * Check if soft keyboard is visible. Does not work on android 7 but does on android 11 avd.
     *
    ...
    /**
     * Check if hardware keyboard is connected. Based on default implementation of {@link
     * InputMethodService#onEvaluateInputViewShown()}.
     *
     * <p>https://developer.android.com/guide/topics/resources/providing-resources#ImeQualifier
     *
     * @param context The Context for operations.
     * @return Returns {@code true} if device has hardware keys for text input or an external
     *     hardware keyboard is connected, otherwise {@code false}.
     */
     ...
     *
     * @param context The Context for operations.
     * @return Returns {@code true} if device has soft keyboard should be disabled, otherwise {@code
     *     false}.
     */
    public static boolean shouldSoftKeyboardBeDisabled(
            final Context context,
            final boolean isSoftKeyboardEnabled,
        ...
        if (!isSoftKeyboardEnabled) {
            ...
            if (isSoftKeyboardEnabledOnlyIfNoHardware) {
                ...
                Logger.logVerbose(
                        LOG_TAG, "Hardware keyboard connected=" + isHardKeyboardConnected);
