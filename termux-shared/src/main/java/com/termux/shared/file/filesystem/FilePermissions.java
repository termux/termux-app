...
/**
 * This class consists exclusively of static methods that operate on sets of {@link FilePermission}
 * objects.
 *
 * <p>https://cs.android.com/android/platform/superproject/+/android-11.0.0_r3:libcore/ojluni/src/main/java/java/nio/file/attribute/PosixFilePermissions.java
 *
 * @since 1.7
 */
public final class FilePermissions {
    ...
    /**
     * Returns the {@code String} representation of a set of permissions. It is guaranteed that the
     * returned {@code String} can be parsed by the {@link #fromString} method.
     *
     * <p>If the set contains {@code null} or elements that are not of type {@code FilePermission}
    ...
    public static String toString(Set<FilePermission> perms) {
        ...
        writeBits(
                sb,
                perms.contains(OWNER_READ),
                perms.contains(OWNER_WRITE),
                perms.contains(OWNER_EXECUTE));
        writeBits(
                sb,
                perms.contains(GROUP_READ),
                perms.contains(GROUP_WRITE),
                perms.contains(GROUP_EXECUTE));
        writeBits(
                sb,
                perms.contains(OTHERS_READ),
     ...
     *
     * <p>The {@code perms} parameter is a {@code String} representing the permissions. It has 9
     * characters that are interpreted as three sets of three. The first set refers to the owner's
     * permissions; the next to the group permissions and the last to others. Within each set, the
     * first character is {@code 'r'} to indicate permission to read, the second character is {@code
     * 'w'} to indicate permission to write, and the third character is {@code 'x'} for execute
     * permission. Where a permission is not set then the corresponding character is set to {@code
     * '-'}.
     *
     * <p><b>Usage Example:</b> Suppose we require the set of permissions that indicate the owner
     * has read, write, and execute permissions, the group has read and execute permissions and
     * others have none.
     *
     * <pre>
