...
public class FileUtilsTests {
     ...
     *
     * <p>Move tests need to be written, specially for failures.
     *
     * <p>The log level must be set to verbose.
     *
     * <p>Run at app startup like in an activity FileUtilsTests.runTests(this,
     * TermuxConstants.TERMUX_HOME_DIR_PATH + "/FileUtilsTests");
     *
     * @param context The {@link Context} for operations.
     */
    public static void runTests(
            @NonNull final Context context, @NonNull final String testRootDirectoryPath) {
        try {
            Logger.logInfo(LOG_TAG, "Running tests");
            Logger.logInfo(LOG_TAG, "testRootDirectoryPath: \"" + testRootDirectoryPath + "\"");

            String fileUtilsTestsDirectoryCanonicalPath =
                    FileUtils.getCanonicalPath(testRootDirectoryPath, null);
            assertEqual(
    ...
    private static void runTestsInner(@NonNull final String testRootDirectoryPath)
            throws Exception {
        Error error;
        String label;
        String path;
        ...
        assertEqual(
                "Failed to copy " + dir1__sub_sym1_label + " symlink file to " + label,
                null,
                error);
        if (!FileUtils.symlinkFileExists(path))
            throwException(
                    "The "
                            + label
                            + " symlink file does not exist as expected after copying it from "
        ...
        assertEqual(
                "Failed to write string to " + label + " file with append mode false", null, error);
        if (!FileUtils.regularFileExists(path, false))
            throwException(
                    "The "
                            + label
                            + " file does not exist as expected after writing to it with append"
                            + " mode false");

        // Write "line2" to dir2/sub_reg1 regular file
        error = FileUtils.writeTextToFile(label, path, Charset.defaultCharset(), "\nline2", true);
        assertEqual(
                "Failed to write string to " + label + " file with append mode true", null, error);

        // Read dir2/sub_reg1 regular file
        StringBuilder dataStringBuilder = new StringBuilder();
        error =
                FileUtils.readTextFromFile(
                        label, path, Charset.defaultCharset(), dataStringBuilder, false);
        ...
        assertEqual(
                "Failed to copy " + dir2__sub_reg1_label + " regular file to " + label,
                null,
                error);
        if (!FileUtils.regularFileExists(path, false))
            throwException(
                    "The "
                            + label
                            + " regular file does not exist as expected after copying it from "
        ...
        if (!FileUtils.directoryFileExists(path, false))
            throwException(
                    "The "
                            + label
                            + " directory file does not exist as expected after copying it from "
                            + dir2_label);
        ...
        // This is to ensure that symlinkFileExists returns true if a symlink file exists but is
        // dangling
        label = dir1__sub_sym3_label;
        path = dir1__sub_sym3_path;
        ...
        if (!FileUtils.symlinkFileExists(path))
            throwException(
                    "The "
                            + label
                            + " dangling symlink file does not exist as expected after creation");
        ...
        // Check if dir2 directory file still exists after deletion of dir1/sub_sym2 since it was a
        // symlink to dir2
        ...
        if (!FileUtils.directoryFileExists(path, false))
            throwException(
                    "The "
                            + label
                            + " directory file has unexpectedly been deleted after deletion of "
        ...
        // When deleting a directory, any targets of symlinks must not be deleted when deleting
        // symlink files
        label = dir2_label;
        path = dir2_path;
        if (!FileUtils.directoryFileExists(path, false))
        ...
        if (!FileUtils.fileExists(path, false))
            throwException(
                    "The "
                            + label
                            + " regular file has unexpectedly been deleted after deletion of "
                            + dir1_label);
        ...
        List<String> ignoredSubFilePaths =
                Arrays.asList(dir1__sub_dir2_path, dir1__sub_dir3__sub_reg1_path);
        ...
        error =
                FileUtils.validateDirectoryFileEmptyOrOnlyContainsSpecificFiles(
                        dir1_label, dir1_path, ignoredSubFilePaths, false);
        assertEqual(
                "Failed to validate if " + dir1_label + " directory file is empty", null, error);
        ...
        assertErrnoEqual(
                "Failed to validate if "
                        + dir1_label
                        + " directory file is empty with parent dir existing of non existing"
                        + " ignored regular file",
                FileUtilsErrno.ERRNO_NON_EMPTY_DIRECTORY_FILE,
                error);
        ...
        assertEqual(
                "Failed to validate if "
                        + dir1_label
                        + " directory file is empty with ignored regular file existing",
                null,
                error);
        ...
        assertEqual(
                "Failed to validate if "
                        + dir1_label
                        + " directory file is empty with ignored dir file existing",
                null,
                error);
        ...
        assertErrnoEqual(
                "Failed to validate if "
                        + dir1_label
                        + " directory file is empty with non ignored dir file existing",
                FileUtilsErrno.ERRNO_NON_EMPTY_DIRECTORY_FILE,
                error);
    ...
    public static void assertEqual(
            @NonNull final String message, final String expected, final Error actual)
            throws Exception {
    ...
    public static void assertEqual(
            @NonNull final String message, final String expected, final String actual)
            throws Exception {
    ...
    public static void assertErrnoEqual(
            @NonNull final String message, final Errno expected, final Error actual)
            throws Exception {
        if ((expected == null && actual != null)
                || (expected != null && !expected.equalsErrorTypeAndCode(actual)))
            throwException(
                    message
                            + "\nexpected: \""
                            + expected
