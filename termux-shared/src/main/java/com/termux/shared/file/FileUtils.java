...
public class FileUtils {
    ...
    /**
     * Required file permissions for the executable file for app usage. Executable file must have
     * read and execute permissions
     */
    public static final String APP_EXECUTABLE_FILE_PERMISSIONS = "r-x"; // Default: "r-x"
    /**
     * Required file permissions for the working directory for app usage. Working directory must
     * have read and write permissions. Execute permissions should be attempted to be set, but
     * ignored if they are missing
     */
     ...
     * Get canonical path.
     *
     * <p>If path is already an absolute path, then it is used as is to get canonical path. If path
     * is not an absolute path and {code prefixForNonAbsolutePath} is not {@code null}, then {code
     * prefixForNonAbsolutePath} + "/" is prefixed before path before getting canonical path. If
     * path is not an absolute path and {code prefixForNonAbsolutePath} is {@code null}, then "/" is
     * prefixed before path before getting canonical path.
     *
     * <p>If an exception is raised to get the canonical path, then absolute path is returned.
     *
     * @param path The {@code path} to convert.
     * @param prefixForNonAbsolutePath Optional prefix path to prefix before non-absolute paths.
     *     This can be set to {@code null} if non-absolute paths should be prefixed with "/". The
     *     call to {@link File#getCanonicalPath()} will automatically do this anyways.
    ...
    /**
     * Removes one or more forward slashes "//" with single slash "/" Removes "./" Removes trailing
     * forward slash "/"
     *
     * @param path The {@code path} to convert.
    ...
    public static String sanitizeFileName(
            String fileName, boolean sanitizeWhitespaces, boolean toLower) {
        if (fileName == null) return null;

        if (sanitizeWhitespaces) fileName = fileName.replaceAll("[\\\\/:*?\"<>| \t\n]", "_");
        else fileName = fileName.replaceAll("[\\\\/:*?\"<>|]", "_");

        if (toLower) return fileName.toLowerCase();
     ...
     * @param ensureUnder If set to {@code true}, then it will be ensured that {@code path} is under
     *     the directory and does not equal it.
     * @return Returns {@code true} if path in {@code dirPath}, otherwise returns {@code false}.
     */
    public static boolean isPathInDirPath(
            String path, final String dirPath, final boolean ensureUnder) {
     ...
     * @param ensureUnder If set to {@code true}, then it will be ensured that {@code path} is under
     *     the directories and does not equal it.
     * @return Returns {@code true} if path in {@code dirPaths}, otherwise returns {@code false}.
     */
    public static boolean isPathInDirPaths(
            String path, final List<String> dirPaths, final boolean ensureUnder) {
        ...
        for (String dirPath : dirPaths) {
            ...
            else isPathInDirPaths = path.startsWith(normalizedDirPath + "/");
     ...
     *
     * <p>If parent path of an ignored file exists, but ignored file itself does not exist, then
     * directory is not considered empty.
     *
     ...
     * @return Returns {@code null} if directory is empty or contains only files in {@code
     *     ignoredSubFilePaths}. Returns {@code FileUtilsErrno#ERRNO_NON_EMPTY_DIRECTORY_FILE} if a
     *     file was found that did not exist in the {@code ignoredSubFilePaths}, otherwise returns
     *     an appropriate {@code error} if checking was not successful.
     */
    public static Error validateDirectoryFileEmptyOrOnlyContainsSpecificFiles(
            String label,
            String filePath,
        ...
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(
                    label + "file path", "isDirectoryFileEmptyOrOnlyContainsSpecificFiles");
        ...
        try {
            ...
            if (fileType != FileType.NO_EXIST && fileType != FileType.DIRECTORY) {
                return FileUtilsErrno.ERRNO_NON_DIRECTORY_FILE_FOUND
                        .getError(label + "directory", filePath)
            ...
            return FileUtilsErrno
                    .ERRNO_VALIDATE_DIRECTORY_EMPTY_OR_ONLY_CONTAINS_SPECIFIC_FILES_FAILED_WITH_EXCEPTION
     ...
     * directory is not considered empty.
     *
     * <p>This function should ideally not be called by itself but through {@link
     * #validateDirectoryFileEmptyOrOnlyContainsSpecificFiles(String, String, List, boolean)}.
     *
     * @param subFiles The list of files of a directory to check.
     * @param ignoredSubFilePaths The list of absolute file paths under {@code filePath} dir.
     *     Validation is done for the paths.
     * @return Returns {@code true} if a file was found that did not exist in the {@code
     *     ignoredSubFilePaths}, otherwise {@code false}.
     */
    public static boolean nonIgnoredSubFileExists(
            File[] subFiles, @NonNull List<String> ignoredSubFilePaths) {
        ...
        for (File subFile : subFiles) {
            ...
            if (!ignoredSubFilePaths.contains(subFilePath)) {
                ...
                for (String ignoredSubFilePath : ignoredSubFilePaths) {
                    if (ignoredSubFilePath.startsWith(subFilePath + "/")
                            && fileExists(ignoredSubFilePath, false)) {
                        isParentPath = true;
                        break;
                    }
                }
     ...
     *     finding if file exists. Check {@link #getFileType(String, boolean)} for details.
     ...
     */
    public static boolean regularOrDirectoryFileExists(
            final String filePath, final boolean followLinks) {
     ...
     *
     * <p>This function is a wrapper for {@link FileTypes#getFileType(String, boolean)}
     *
     * @param filePath The {@code path} for file to check.
     * @param followLinks The {@code boolean} that decides if symlinks will be followed while
     *     finding type. If set to {@code true}, then type of symlink target will be returned if
     *     file at {@code filePath} is a symlink. If set to {@code false}, then type of file at
     *     {@code filePath} itself will be returned.
     ...
     *
     * <p>If the {@code parentDirPath} is not {@code null}, then setting of missing permissions will
     ...
     * @param permissionsToCheck The 3 character string that contains the "r", "w", "x" or "-"
     *     in-order.
     * @param setPermissions The {@code boolean} that decides if permissions are to be automatically
     *     set defined by {@code permissionsToCheck}.
     ...
     *     errors are to be ignored if path is under {@code parentDirPath}.
     * @return Returns the {@code error} if path is not a regular file, or validating permissions
     *     failed, otherwise {@code null}.
     */
    public static Error validateRegularFileExistenceAndPermissions(
            String label,
            final String filePath,
            final String parentDirPath,
            final String permissionsToCheck,
            final boolean setPermissions,
            final boolean setMissingPermissionsOnly,
            final boolean ignoreErrorsIfPathIsUnderParentDirPath) {
        label = (label == null || label.isEmpty() ? "" : label + " ");
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(
                    label + "regular file path", "validateRegularFileExistenceAndPermissions");
        ...
        try {
            ...
            if (fileType != FileType.NO_EXIST && fileType != FileType.REGULAR) {
                return FileUtilsErrno.ERRNO_NON_REGULAR_FILE_FOUND
                        .getError(label + "file", filePath)
                        .setLabel(label + "file");
            }
            ...
            if (setPermissions && permissionsToCheck != null && fileType == FileType.REGULAR) {
                ...
                if (parentDirPath == null
                        || (isPathUnderParentDirPath
                                && getFileType(parentDirPath, false) == FileType.DIRECTORY)) {
            ...
            if (parentDirPath == null
                ...
                if (permissionsToCheck != null) {
                    ...
                    return checkMissingFilePermissions(
                            label + "regular", filePath, permissionsToCheck, false);
                }
            }
        } catch (Exception e) {
            return FileUtilsErrno
                    .ERRNO_VALIDATE_FILE_EXISTENCE_AND_PERMISSIONS_FAILED_WITH_EXCEPTION
                    .getError(e, label + "file", filePath, e.getMessage());
        }
     ...
     *
     * <p>If the {@code parentDirPath} is not {@code null}, then creation of missing directory and
     * setting of missing permissions will only be done if {@code path} is under {@code
     * parentDirPath} or equals {@code parentDirPath}.
     *
     * @param label The optional label for the directory file. This can optionally be {@code null}.
     * @param filePath The {@code path} for file to validate or create. Symlinks will not be
     *     followed.
     * @param parentDirPath The optional {@code parent directory path} to restrict operations to.
     *     This can optionally be {@code null}. It is not canonicalized and only normalized.
     * @param createDirectoryIfMissing The {@code boolean} that decides if directory file should be
     *     created if its missing.
     * @param permissionsToCheck The 3 character string that contains the "r", "w", "x" or "-"
     *     in-order.
     * @param setPermissions The {@code boolean} that decides if permissions are to be automatically
     *     set defined by {@code permissionsToCheck}.
     * @param setMissingPermissionsOnly The {@code boolean} that decides if only missing permissions
     *     are to be set or if they should be overridden.
     * @param ignoreErrorsIfPathIsInParentDirPath The {@code boolean} that decides if existence and
     *     permission errors are to be ignored if path is in {@code parentDirPath}.
     * @param ignoreIfNotExecutable The {@code boolean} that decides if missing executable
     *     permission error is to be ignored. This allows making an attempt to set executable
     *     permissions, but ignoring if it fails.
     * @return Returns the {@code error} if path is not a directory file, failed to create it, or
     *     validating permissions failed, otherwise {@code null}.
     */
    public static Error validateDirectoryFileExistenceAndPermissions(
            String label,
            final String filePath,
            final String parentDirPath,
            ...
            final boolean setPermissions,
            final boolean setMissingPermissionsOnly,
            final boolean ignoreErrorsIfPathIsInParentDirPath,
            final boolean ignoreIfNotExecutable) {
        label = (label == null || label.isEmpty() ? "" : label + " ");
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(
                    label + "directory file path", "validateDirectoryExistenceAndPermissions");
        ...
        try {
            ...
            if (createDirectoryIfMissing || setPermissions) {
                ...
                if (parentDirPath == null
                        || (isPathInParentDirPath
                                && getFileType(parentDirPath, false) == FileType.DIRECTORY)) {
                    // If createDirectoryIfMissing is enabled and no file exists at path, then
                    // create directory
                    if (createDirectoryIfMissing && fileType == FileType.NO_EXIST) {
                        Logger.logVerbose(
                                ...
                                "Creating "
                                        ...
                                        + filePath
                                        + "\"");
                        // Create directory and update fileType if successful, otherwise return with
                        // error
                        // It "might" be possible that mkdirs returns false even though directory
                        // was created
                        boolean result = file.mkdirs();
                        fileType = getFileType(filePath, false);
                        if (!result && fileType != FileType.DIRECTORY)
                            return FileUtilsErrno.ERRNO_CREATING_FILE_FAILED.getError(
                                    label + "directory file", filePath);
                    }
                    ...
                    if (setPermissions
                        ...
                        if (setMissingPermissionsOnly)
                            setMissingFilePermissions(
                                    label + "directory", filePath, permissionsToCheck);
            ...
            if (parentDirPath == null
                ...
                // Directories can be automatically created so we can ignore if missing with above
                // check
            ...
            return FileUtilsErrno
                    .ERRNO_VALIDATE_DIRECTORY_EXISTENCE_AND_PERMISSIONS_FAILED_WITH_EXCEPTION
                    .getError(e, label + "directory file", filePath, e.getMessage());
        }
     ...
     *
     * <p>This function is a wrapper for {@link
     * #validateDirectoryFileExistenceAndPermissions(String, String, String, boolean, String,
     ...
     * Create a regular file at path.
     *
     * <p>This function is a wrapper for {@link #validateRegularFileExistenceAndPermissions(String,
     ...
     * @param setMissingPermissionsOnly The {@code boolean} that decides if only missing permissions
     *     are to be set or if they should be overridden.
     * @return Returns the {@code error} if path is not a regular file, failed to create it, or
     *     validating permissions failed, otherwise {@code null}.
     */
    public static Error createRegularFile(
            String label,
            final String filePath,
            final String permissionsToCheck,
            final boolean setPermissions,
        ...
        try {
            Logger.logVerbose(
                    LOG_TAG, "Creating " + label + "regular file at path \"" + filePath + "\"");

            if (!file.createNewFile())
                return FileUtilsErrno.ERRNO_CREATING_FILE_FAILED.getError(
                        label + "regular file", filePath);
        } catch (Exception e) {
            return FileUtilsErrno.ERRNO_CREATING_FILE_FAILED_WITH_EXCEPTION.getError(
                    e, label + "regular file", filePath, e.getMessage());
        }
     ...
     * boolean, boolean, boolean, boolean)}.
     *
     * @param label The optional label for the parent directory file. This can optionally be {@code
     *     null}.
     * @param filePath The {@code path} for file whose parent needs to be created.
     * @return Returns the {@code error} if parent path is not a directory file or failed to create
     *     it, otherwise {@code null}.
     */
    public static Error createParentDirectoryFile(final String label, final String filePath) {
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(
                    label + "file path", "createParentDirectoryFile");
     ...
     *     validating permissions failed, otherwise {@code null}.
     */
    public static Error createDirectoryFile(
            final String label,
            final String filePath,
            final String permissionsToCheck,
            final boolean setPermissions,
     ...
     *
     * <p>This function is a wrapper for {@link #createSymlinkFile(String, String, String, boolean,
     * boolean, boolean)}.
     *
     * <p>Dangling symlinks will be allowed. Symlink destination will be overwritten if it already
     * exists but only if its a symlink.
     *
     ...
     *     otherwise {@code null}.
     */
    public static Error createSymlinkFile(
            String label, final String targetFilePath, final String destFilePath) {
     ...
     * @param targetFilePath The {@code path} TO which the symlink file will be created.
     * @param destFilePath The {@code path} AT which the symlink file will be created.
     * @param allowDangling The {@code boolean} that decides if it should be considered an error if
     *     source file doesn't exist.
     * @param overwrite The {@code boolean} that decides if destination file should be overwritten
     *     if it already exists. If set to {@code true}, then destination file will be deleted
     *     before symlink is created.
     ...
     * @return Returns the {@code error} if path is not a symlink file, failed to create it, or
     *     validating permissions failed, otherwise {@code null}.
     */
    public static Error createSymlinkFile(
            String label,
            ...
            final boolean allowDangling,
            final boolean overwrite,
            final boolean overwriteOnlyIfDestIsASymlink) {
        label = (label == null || label.isEmpty() ? "" : label + " ");
        if (targetFilePath == null || targetFilePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(
                    label + "target file path", "createSymlinkFile");
        if (destFilePath == null || destFilePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(
                    label + "destination file path", "createSymlinkFile");
        ...
        try {
            ...
            if (targetFileType == FileType.NO_EXIST) {
                ...
                if (!allowDangling) {
                    ...
                    return FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH
                            .getError(label, targetFileAbsolutePath)
                            .setLabel(label);
                }
            }
            ...
            if (destFileType != FileType.NO_EXIST) {
                ...
                error =
                        createParentDirectoryFile(
                                label + "symlink destination file parent", destFilePath);
                if (error != null) return error;
            }
            ...
            return FileUtilsErrno.ERRNO_CREATING_SYMLINK_FILE_FAILED_WITH_EXCEPTION.getError(
                    e, label + "symlink file", destFilePath, targetFilePath, e.getMessage());
        }
     ...
     *
     * <p>This function is a wrapper for {@link #copyOrMoveFile(String, String, String, boolean,
     * boolean, int, boolean, boolean)}.
     *
     * <p>If destination file already exists, then it will be overwritten, but only if its a regular
     ...
     * @param ignoreNonExistentSrcFile The {@code boolean} that decides if it should be considered
     *     an error if source file to copied doesn't exist.
     * @return Returns the {@code error} if copy was not successful, otherwise {@code null}.
     */
    public static Error copyRegularFile(
            final String label,
     ...
     * @param ignoreNonExistentSrcFile The {@code boolean} that decides if it should be considered
     *     an error if source file to moved doesn't exist.
     * @return Returns the {@code error} if move was not successful, otherwise {@code null}.
     */
    public static Error moveRegularFile(
            final String label,
            final String srcFilePath,
            final String destFilePath,
            final boolean ignoreNonExistentSrcFile) {
     ...
     * boolean, int, boolean, boolean)}.
     *
     * <p>If destination file already exists, then it will be overwritten, but only if its a
     * directory file, otherwise an error will be returned.
     *
     * @param label The optional label for file to copy. This can optionally be {@code null}.
     * @param srcFilePath The {@code source path} for file to copy.
     * @param destFilePath The {@code destination path} for file to copy.
     ...
     * @return Returns the {@code error} if copy was not successful, otherwise {@code null}.
     */
    public static Error copyDirectoryFile(
            final String label,
            final String srcFilePath,
            final String destFilePath,
            final boolean ignoreNonExistentSrcFile) {
     ...
     * @return Returns the {@code error} if move was not successful, otherwise {@code null}.
     */
    public static Error moveDirectoryFile(
            final String label,
            final String srcFilePath,
            final String destFilePath,
            final boolean ignoreNonExistentSrcFile) {
     ...
     * boolean, int, boolean, boolean)}.
     *
     * <p>If destination file already exists, then it will be overwritten, but only if its a symlink
     * file, otherwise an error will be returned.
     *
     * @param label The optional label for file to copy. This can optionally be {@code null}.
     * @param srcFilePath The {@code source path} for file to copy.
     ...
     * @return Returns the {@code error} if move was not successful, otherwise {@code null}.
     */
    public static Error moveSymlinkFile(
            final String label,
            final String srcFilePath,
            final String destFilePath,
            final boolean ignoreNonExistentSrcFile) {
     ...
     * boolean, int, boolean, boolean)}.
     *
     * <p>If destination file already exists, then it will be overwritten, but only if its the same
     * file type as the source, otherwise an error will be returned.
     *
     * @param label The optional label for file to copy. This can optionally be {@code null}.
     * @param srcFilePath The {@code source path} for file to copy.
     * @param destFilePath The {@code destination path} for file to copy.
    ...
    public static Error copyFile(
        ...
        return copyOrMoveFile(
                ...
                false,
                ignoreNonExistentSrcFile,
                FileTypes.FILE_TYPE_NORMAL_FLAGS,
                true,
                true);
    }
     ...
     *
     * <p>The {@code sourceFilePath} and {@code destFilePath} must be the canonical path to the
     * source and destination since symlinks will not be followed.
     *
     * <p>If the {@code sourceFilePath} or {@code destFilePath} is a canonical path to a directory,
     ...
     *
     * @param label The optional label for file to copy or move. This can optionally be {@code
     *     null}.
     ...
     *     If set to {@code true}, then source file will be moved, otherwise it will be copied.
     * @param ignoreNonExistentSrcFile The {@code boolean} that decides if it should be considered
     *     an error if source file to copied or moved doesn't exist.
     * @param allowedFileTypeFlags The flags that are matched against the source file's {@link
     *     FileType} to see if it should be copied/moved or not. This is a safety measure to prevent
     *     accidental copy/move/delete of the wrong type of file, like a directory instead of a
     *     regular file. You can pass {@link FileTypes#FILE_TYPE_ANY_FLAGS} to allow copy/move of
     *     any file type.
     * @param overwrite The {@code boolean} that decides if destination file should be overwritten
     *     if it already exists. If set to {@code true}, then destination file will be deleted
     *     before source is copied or moved.
     * @param overwriteOnlyIfDestSameFileTypeAsSrc The {@code boolean} that decides if overwrite
     *     should only be done if destination file is also the same file type as the source file.
     * @return Returns the {@code error} if copy or move was not successful, otherwise {@code null}.
     */
    public static Error copyOrMoveFile(
            String label,
            final String srcFilePath,
            final String destFilePath,
            final boolean moveFile,
            final boolean ignoreNonExistentSrcFile,
            int allowedFileTypeFlags,
            final boolean overwrite,
            final boolean overwriteOnlyIfDestSameFileTypeAsSrc) {
        label = (label == null || label.isEmpty() ? "" : label + " ");
        if (srcFilePath == null || srcFilePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(
                    label + "source file path", "copyOrMoveFile");
        if (destFilePath == null || destFilePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(
                    label + "destination file path", "copyOrMoveFile");
        ...
        try {
            ...
            if (srcFileType == FileType.NO_EXIST) {
                ...
                if (ignoreNonExistentSrcFile) return null;
                // Else return with error
                else {
                    label += "source file";
                    return FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH
                            .getError(label, srcFilePath)
                            .setLabel(label);
                }
            }

            // If the file type of the source file does not exist in the allowedFileTypeFlags, then
            // return with error
            ...
            if (srcFileCanonicalPath.equals(destFileCanonicalPath))
                return FileUtilsErrno.ERRNO_COPYING_OR_MOVING_FILE_TO_SAME_PATH.getError(
                        mode + " " + label + "source file", srcFilePath, destFilePath);
            ...
            if (destFileType != FileType.NO_EXIST) {
                ...
                // If overwriteOnlyIfDestSameFileTypeAsSrc is enabled but destination file does not
                // match source file type
            ...
            // If moveFile is true
            if (moveFile) {
                // We first try to rename source file to destination file to save a copy operation
                // in case both source and destination are on the same filesystem
                ...
                // Uses File.getPath() to get the path of source and destination and not the
                // canonical path
                if (!srcFile.renameTo(destFile)) {
                    // If destination directory is a subdirectory of the source directory
                    // Copying is still allowed by copyDirectory() by excluding destination
                    // directory files
                    if (srcFileType == FileType.DIRECTORY
                            && destFileCanonicalPath.startsWith(
                    ...
                    Logger.logVerbose(
                            ...
                            "Renaming "
                                    + label
                                    + "source file to destination failed, attempting to copy.");
                    copyFile = true;
                }
            }
            ...
            if (copyFile) {
                ...
                if (srcFileType == FileType.DIRECTORY) {
                    // Will give runtime exceptions on android < 8 due to missing classes like
                    // java.nio.file.Path if org.apache.commons.io version > 2.5
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                        ...
                        error =
                                createSymlinkFile(
                                        label + "dest", Os.readlink(srcFilePath), destFilePath);
                        if (error != null) return error;
                    }
                } else {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            return FileUtilsErrno.ERRNO_COPYING_OR_MOVING_FILE_FAILED_WITH_EXCEPTION.getError(
                    e, mode + " " + label + "file", srcFilePath, destFilePath, e.getMessage());
        }
     ...
     *
     * <p>This function is a wrapper for {@link #deleteFile(String, String, boolean, boolean, int)}.
     *
     ...
     */
    public static Error deleteRegularFile(
            String label, final String filePath, final boolean ignoreNonExistentFile) {
        return deleteFile(
                label, filePath, ignoreNonExistentFile, false, FileType.REGULAR.getValue());
    }
     ...
     * @return Returns the {@code error} if deletion was not successful, otherwise {@code null}.
     */
    public static Error deleteDirectoryFile(
            String label, final String filePath, final boolean ignoreNonExistentFile) {
        return deleteFile(
     ...
     * @return Returns the {@code error} if deletion was not successful, otherwise {@code null}.
     */
    public static Error deleteSymlinkFile(
            String label, final String filePath, final boolean ignoreNonExistentFile) {
        return deleteFile(
                label, filePath, ignoreNonExistentFile, false, FileType.SYMLINK.getValue());
    }
     ...
     * @return Returns the {@code error} if deletion was not successful, otherwise {@code null}.
     */
    public static Error deleteSocketFile(
            String label, final String filePath, final boolean ignoreNonExistentFile) {
        return deleteFile(
                label, filePath, ignoreNonExistentFile, false, FileType.SOCKET.getValue());
    }
     ...
     * @return Returns the {@code error} if deletion was not successful, otherwise {@code null}.
     */
    public static Error deleteFile(
            String label, final String filePath, final boolean ignoreNonExistentFile) {
        return deleteFile(
     ...
     * Delete file at path.
     *
     * <p>The {@code filePath} must be the canonical path to the file to be deleted since symlinks
     * will not be followed. If the {@code filePath} is a canonical path to a directory, then any
     * symlink files found under the directory will be deleted, but not their targets.
     *
     * @param label The optional label for file to delete. This can optionally be {@code null}.
     * @param filePath The {@code path} for file to delete.
     * @param ignoreNonExistentFile The {@code boolean} that decides if it should be considered an
     ...
     *     see if it should be deleted or not. This is a safety measure to prevent accidental
     *     deletion of the wrong type of file, like a directory instead of a regular file. You can
     *     pass {@link FileTypes#FILE_TYPE_ANY_FLAGS} to allow deletion of any file type.
     * @return Returns the {@code error} if deletion was not successful, otherwise {@code null}.
     */
    public static Error deleteFile(
            String label,
            final String filePath,
            final boolean ignoreNonExistentFile,
            final boolean ignoreWrongFileType,
            int allowedFileTypeFlags) {
        label = (label == null || label.isEmpty() ? "" : label + " ");
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(
                    label + "file path", "deleteFile");
        ...
        try {
                ...
                if (ignoreWrongFileType) {
                    Logger.logVerbose(
                            ...
                            "Ignoring deletion of "
                                    ...
                                    + FileTypes.convertFileTypeFlagsToNamesString(
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ...
                com.google.common.io.MoreFiles.deleteRecursively(
                        file.toPath(), RecursiveDeleteOption.ALLOW_INSECURE);
            } else {
                if (fileType == FileType.DIRECTORY) {
                    // deleteDirectory() instead of forceDelete() gets the files list first instead
                    // of walking directory tree, so seems safer
                    // Will give runtime exceptions on android < 8 due to missing classes like
                    // java.nio.file.Path if org.apache.commons.io version > 2.5
            ...
            if (fileType != FileType.NO_EXIST)
                return FileUtilsErrno.ERRNO_FILE_STILL_EXISTS_AFTER_DELETING.getError(
                        label + "file meant to be deleted", filePath);
        } catch (Exception e) {
            return FileUtilsErrno.ERRNO_DELETING_FILE_FAILED_WITH_EXCEPTION.getError(
                    e, label + "file", filePath, e.getMessage());
        }
    ...
    /**
     * Clear contents of directory at path without deleting the directory. If directory does not
     * exist it will be created automatically.
     *
     * <p>This function is a wrapper for {@link #clearDirectory(String, String)}.
     *
     ...
     * exist it will be created automatically.
     *
     * <p>The {@code filePath} must be the canonical path to a directory since symlinks will not be
     * followed. Any symlink files found under the directory will be deleted, but not their targets.
     *
    ...
    public static Error clearDirectory(String label, final String filePath) {
        ...
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(
                    label + "file path", "clearDirectory");

        Error error;

        try {
            Logger.logVerbose(
                    LOG_TAG, "Clearing " + label + "directory at path \"" + filePath + "\"");
            ...
            return FileUtilsErrno.ERRNO_CLEARING_DIRECTORY_FAILED_WITH_EXCEPTION.getError(
                    e, label + "directory", filePath, e.getMessage());
        }
     ...
     * @param label The optional label for directory to clear. This can optionally be {@code null}.
     * @param filePath The {@code path} for directory to clear.
     * @param dirFilter The optional filter to apply when finding subdirectories. If this parameter
     *     is {@code null}, subdirectories will not be included in the search. Use
     ...
     */
    public static Error deleteFilesOlderThanXDays(
            String label,
            final String filePath,
        ...
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(
                    label + "file path", "deleteFilesOlderThanXDays");
        if (days < 0)
            return FunctionErrno.ERRNO_INVALID_PARAMETER.getError(
                    label + "days", "deleteFilesOlderThanXDays", " It must be >= 0.");
        ...
        try {
            ...
            // TODO: Use FileAttributes with support for atime (default), mtime, ctime. Add regex
            // for ignoring file and dir absolute paths.
            // FIXME: iterateFiles() does not return subdirectories even with TrueFileFilter for
            // file and dir.
            ...
            // AgeFileFilter seems to apply to symlink destination timestamp instead of symlink file
            // itself
            Iterator<File> filesToDelete =
                    org.apache.commons.io.FileUtils.iterateFiles(
                            file, new AgeFileFilter(calendar.getTime()), dirFilter);
            ...
            return FileUtilsErrno.ERRNO_DELETING_FILES_OLDER_THAN_X_DAYS_FAILED_WITH_EXCEPTION
                    .getError(e, label + "directory", filePath, days, e.getMessage());
        }
    ...
    /**
     * Read a text {@link String} from file at path with a specific {@link Charset} into {@code
     * dataString}.
     *
     ...
     * @param charset The {@link Charset} of the file. If this is {@code null}, then default {@link
     ...
     */
    public static Error readTextFromFile(
            String label,
            final String filePath,
            Charset charset,
            @NonNull final StringBuilder dataStringBuilder,
            final boolean ignoreNonExistentFile) {
        label = (label == null || label.isEmpty() ? "" : label + " ");
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(
                    label + "file path", "readStringFromFile");

        Logger.logVerbose(
                LOG_TAG, "Reading text from " + label + "file at path \"" + filePath + "\"");
        ...
        try {
            ...
            return FileUtilsErrno.ERRNO_READING_TEXT_FROM_FILE_FAILED_WITH_EXCEPTION.getError(
                    e, label + "file", filePath, e.getMessage());
        } finally {
    ...
    public static <T extends Serializable>
            ReadSerializableObjectResult readSerializableObjectFromFile(
                    String label,
                    final String filePath,
                    Class<T> readObjectType,
                    final boolean ignoreNonExistentFile) {
        label = (label == null || label.isEmpty() ? "" : label + " ");
        if (filePath == null || filePath.isEmpty())
            return new ReadSerializableObjectResult(
                    FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(
                            label + "file path", "readSerializableObjectFromFile"),
                    null);
        ...
        if (fileType != FileType.NO_EXIST && fileType != FileType.REGULAR) {
            return new ReadSerializableObjectResult(
                    FileUtilsErrno.ERRNO_NON_REGULAR_FILE_FOUND
                            .getError(label + "file", filePath)
                            .setLabel(label + "file"),
                    null);
        }
        ...
        if (fileType == FileType.NO_EXIST) {
            // If reading is to be ignored if file does not exist
            if (ignoreNonExistentFile) return new ReadSerializableObjectResult(null, null);
            // Else return with error
            else {
                label += "file meant to be read";
                return new ReadSerializableObjectResult(
        ...
        try {
            ...
            // Logger.logVerbose(LOG_TAG, Logger.getMultiLineLogStringEntry("String",
            // DataUtils.getTruncatedCommandOutput(dataStringBuilder.toString(),
            // Logger.LOGGER_ENTRY_MAX_SAFE_PAYLOAD, true, false, true), "-"));
        } catch (Exception e) {
            return new ReadSerializableObjectResult(
     ...
     * @param charset The {@link Charset} of the {@code dataString}. If this is {@code null}, then
     ...
     */
    public static Error writeTextToFile(
            String label,
            final String filePath,
            Charset charset,
        ...
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(
                    label + "file path", "writeStringToFile");

        Logger.logVerbose(
                LOG_TAG,
                Logger.getMultiLineLogStringEntry(
                        "Writing text to " + label + "file at path \"" + filePath + "\"",
                        DataUtils.getTruncatedCommandOutput(
                                dataString,
                                Logger.LOGGER_ENTRY_MAX_SAFE_PAYLOAD,
                                true,
     ...
     * @return Returns the {@code error} if writing was not successful, otherwise {@code null}.
     */
    public static <T extends Serializable> Error writeSerializableObjectToFile(
            String label, final String filePath, final T serializableObject) {
        label = (label == null || label.isEmpty() ? "" : label + " ");
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(
                    label + "file path", "writeSerializableObjectToFile");
     ...
     * @return Returns the {@code error} if charset is not supported or failed to check it,
     *     otherwise {@code null}.
     */
    public static Error isCharsetSupported(final Charset charset) {
        if (charset == null)
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(
                    "charset", "isCharsetSupported");
     ...
     * @param permissionsToSet The 3 character string that contains the "r", "w", "x" or "-"
     *     in-order.
     */
    ...
    public static void setFilePermissions(
            String label, final String filePath, final String permissionsToSet) {
        label = (label == null || label.isEmpty() ? "" : label + " ");
        ...
        if (!isValidPermissionString(permissionsToSet)) {
            Logger.logError(
                    LOG_TAG,
                    "Invalid permissionsToSet passed to setFilePermissions: \""
                            + permissionsToSet
                            + "\"");
            return;
        }
        ...
        if (permissionsToSet.contains("w")) {
            ...
            if (file.canWrite()) {
                Logger.logVerbose(
                        LOG_TAG,
                        "Removing write permissions for "
                                + label
                                + "file at path \""
                                + filePath
                                + "\"");
        ...
        if (permissionsToSet.contains("x")) {
            if (!file.canExecute()) {
                Logger.logVerbose(
                        LOG_TAG,
                        "Setting execute permissions for "
                                + label
                                + "file at path \""
                                + filePath
                                + "\"");
            ...
            if (file.canExecute()) {
                Logger.logVerbose(
                        LOG_TAG,
                        "Removing execute permissions for "
                                + label
                                + "file at path \""
                                + filePath
                                + "\"");
    ...
    /**
     * Set missing permissions for file at path. Existing permission outside the {@code
     * permissionsToSet} will not be removed.
     *
     * @param filePath The {@code path} for file to set permissions to.
     * @param permissionsToSet The 3 character string that contains the "r", "w", "x" or "-"
     *     in-order.
     */
    public static void setMissingFilePermissions(
            final String filePath, final String permissionsToSet) {
    ...
    public static void setMissingFilePermissions(
        ...
        if (!isValidPermissionString(permissionsToSet)) {
            Logger.logError(
                    LOG_TAG,
                    "Invalid permissionsToSet passed to setMissingFilePermissions: \""
                            + permissionsToSet
                            + "\"");
            return;
        }
        ...
        if (permissionsToSet.contains("r") && !file.canRead()) {
            Logger.logVerbose(
                    LOG_TAG,
                    "Setting missing read permissions for "
                            + label
                            + "file at path \""
                            + filePath
                            + "\"");
        ...
        if (permissionsToSet.contains("w") && !file.canWrite()) {
            Logger.logVerbose(
                    LOG_TAG,
                    "Setting missing write permissions for "
                            + label
                            + "file at path \""
                            + filePath
                            + "\"");
        ...
        if (permissionsToSet.contains("x") && !file.canExecute()) {
            Logger.logVerbose(
                    LOG_TAG,
                    "Setting missing execute permissions for "
                            + label
                            + "file at path \""
                            + filePath
                            + "\"");
    ...
    public static Error checkMissingFilePermissions(
        ...
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(
                    label + "file path", "checkMissingFilePermissions");

        if (!isValidPermissionString(permissionsToCheck)) {
            Logger.logError(
                    LOG_TAG,
                    "Invalid permissionsToCheck passed to checkMissingFilePermissions: \""
                            + permissionsToCheck
                            + "\"");
        ...
        if (permissionsToCheck.contains("r") && !file.canRead()) {
            return FileUtilsErrno.ERRNO_FILE_NOT_READABLE
                    .getError(label + "file", filePath)
                    .setLabel(label + "file");
        }
        ...
        if (permissionsToCheck.contains("w") && !file.canWrite()) {
            return FileUtilsErrno.ERRNO_FILE_NOT_WRITABLE
                    .getError(label + "file", filePath)
                    .setLabel(label + "file");
        }
        ...
        // This canExecute() will give "avc: granted { execute }" warnings for target sdk 29
            ...
            return FileUtilsErrno.ERRNO_FILE_NOT_EXECUTABLE
                    .getError(label + "file", filePath)
                    .setLabel(label + "file");
        }
    ...
    /**
     * Checks whether string exactly matches the 3 character permission string that contains the
     * "r", "w", "x" or "-" in-order.
     *
     * @param string The {@link String} to check.
     * @return Returns {@code true} if string exactly matches a permission string, otherwise {@code
     *     false}.
     */
    ...
    public static Error getShortFileUtilsError(final Error error) {
        ...
        Errno shortErrno =
                FileUtilsErrno.ERRNO_SHORT_MAPPING.get(Errno.valueOf(type, error.getCode()));
        ...
        else return shortErrno.getError(throwables, error.getLabel(), "file");
    }
