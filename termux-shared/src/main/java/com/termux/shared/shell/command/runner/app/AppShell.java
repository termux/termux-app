...
/**
 * A class that maintains info for background app shells run with {@link Runtime#exec(String[],
 * String[], File)}. It also provides a way to link each {@link Process} with the {@link
 * ExecutionCommand} that started it. The shell is run in the app user context.
 */
public final class AppShell {
     ...
     * File)}.
     *
     * <p>The {@link ExecutionCommand#executable}, must be set. The {@link
     ...
     *     command.
     * @param appShellClient The {@link AppShellClient} interface implementation. The {@link
     *     AppShellClient#onAppShellExited(AppShell)} will be called regardless of {@code
     *     isSynchronous} value but not if {@code null} is returned by this method. This can
     ...
     *     thread and results returned synchronously in the {@link ExecutionCommand} sub object of
     *     the {@link AppShell} returned. If set to {@code false}, then a new thread is started run
     *     the commands asynchronously in the background and control is returned to the caller
     *     thread.
     * @return Returns the {@link AppShell}. This will be {@code null} if failed to start the
     *     execution command.
     */
    public static AppShell execute(
        ...
        Logger.logDebugExtended(
                LOG_TAG,
                ExecutionCommand.getExecutionInputLogString(
                        executionCommand,
                        true,
     ...
     *
     * <p>If the processes finishes, then sets {@link ResultData#stdout}, {@link ResultData#stderr}
     * and {@link ResultData#exitCode} for the {@link #mExecutionCommand} of the {@code appShell}
     * and then calls {@link #processAppShellResult(AppShell, ExecutionCommand) to process the
     * result}.
     *
     * @param context The {@link Context} for operations.
     */
    private void executeInner(@NonNull final Context context)
        ...
        StreamGobbler STDOUT =
                new StreamGobbler(
                        mExecutionCommand.mPid + "-stdout",
                        mProcess.getInputStream(),
                        mExecutionCommand.resultData.stdout,
                        mExecutionCommand.backgroundCustomLogLevel);
        StreamGobbler STDERR =
                new StreamGobbler(
                        mExecutionCommand.mPid + "-stderr",
                        mProcess.getErrorStream(),
                        mExecutionCommand.resultData.stderr,
                        mExecutionCommand.backgroundCustomLogLevel);
            ...
            try {
                ...
                if (e.getMessage() != null
                    ...
                    mExecutionCommand.setStateFailed(
                            ...
                            context.getString(
                                    R.string
                                            .error_exception_received_while_executing_app_shell_command,
                                    mExecutionCommand.getCommandIdAndLabelLogString(),
                                    e.getMessage()),
                            e);
        ...
        if (mExecutionCommand.isStateFailed()) {
            Logger.logDebug(
                    ...
                    "Ignoring setting \""
                            + mExecutionCommand.getCommandIdAndLabelLogString()
                            + "\" AppShell state to ExecutionState.EXECUTED and processing results"
                            + " since it has already failed");
            return;
        }

        mExecutionCommand.resultData.exitCode = exitCode;

        if (!mExecutionCommand.setState(ExecutionState.EXECUTED)) return;
     ...
     *
     * @param context The {@link Context} for operations.
     * @param processResult If set to {@code true}, then the {@link #processAppShellResult(AppShell,
     *     ExecutionCommand)} will be called to process the failure.
     */
     ...
     *
     * <p>Only one of {@code appShell} and {@code executionCommand} must be set.
     *
     * <p>If the {@code appShell} and its {@link #mAppShellClient} are not {@code null}, then the
     * {@link AppShellClient#onAppShellExited(AppShell)} callback will be called.
     *
     * @param appShell The {@link AppShell}, which should be set if {@link #execute(Context,
     *     ExecutionCommand, AppShellClient, IShellEnvironment, HashMap, boolean)} successfully
     *     started the process.
     * @param executionCommand The {@link ExecutionCommand}, which should be set if {@link
     *     #execute(Context, ExecutionCommand, AppShellClient, IShellEnvironment, HashMap, boolean)}
     *     failed to start the process.
     */
    private static void processAppShellResult(
            final AppShell appShell, ExecutionCommand executionCommand) {
        if (appShell != null) executionCommand = appShell.mExecutionCommand;
