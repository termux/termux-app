...
public class ActivityUtils {
     ...
     *     in addition to logging a message. The {@code context} must not be {@code null}.
     * @return Returns the {@code error} if starting activity was not successful, otherwise {@code
     *     null}.
     */
    public static Error startActivity(
            Context context,
        ...
        Error error;
        String activityName =
                intent.getComponent() != null ? intent.getComponent().getClassName() : "Unknown";
        ...
        try {
            ...
            error =
                    ActivityErrno.ERRNO_START_ACTIVITY_FAILED_WITH_EXCEPTION.getError(
                            e, activityName, e.getMessage());
            if (logErrorMessage)
    ...
    /**
     * Wrapper for {@link #startActivityForResult(Context, int, Intent, boolean, boolean,
    ...
    public static Error startActivityForResult(
        ...
        return startActivityForResult(
                context, requestCode, intent, logErrorMessage, showErrorMessage, null);
    }
     ...
     *     {@link AppCompatActivity}. It is ignored if {@code activityResultLauncher} is not {@code
     *     null}.
     * @param requestCode The request code to use while sending intent. This must be >= 0, otherwise
     *     exception will be raised. This is ignored if {@code activityResultLauncher} is {@code
     *     null}.
     * @param intent The {@link Intent} to send to start the activity.
     * @param logErrorMessage If an error message should be logged if failed to start activity.
     * @param showErrorMessage If an error message toast should be shown if failed to start activity
     ...
     *     activity. If this is {@code null}, then {@link Activity#startActivityForResult(Intent,
     *     int)} will be used instead. Note that later is deprecated.
     * @return Returns the {@code error} if starting activity was not successful, otherwise {@code
     *     null}.
     */
    public static Error startActivityForResult(
        ...
        try {
            if (activityResultLauncher != null) {
                ...
                if (context == null) {
                    error =
                            ActivityErrno.ERRNO_STARTING_ACTIVITY_WITH_NULL_CONTEXT.getError(
                                    activityName);
                    if (logErrorMessage) error.logErrorAndShowToast(null, LOG_TAG);
                    return error;
                }
            ...
            error =
                    ActivityErrno.ERRNO_START_ACTIVITY_FOR_RESULT_FAILED_WITH_EXCEPTION.getError(
                            e, activityName, e.getMessage());
            if (logErrorMessage)
                error.logErrorAndShowToast(showErrorMessage ? context : null, LOG_TAG);
            return error;
