...
public class AppCompatActivityUtils {
     ...
     * @param local If set to {@code true}, then a call to {@link
     *     AppCompatDelegate#setLocalNightMode(int)} will be made, otherwise to {@link
    ...
    public static void setToolbarTitle(
        ...
        if (toolbar != null) {
            ...
            try {
                if (titleAppearance != 0) toolbar.setTitleTextAppearance(activity, titleAppearance);
            } catch (Exception e) {
                Logger.logStackTraceWithMessage(
                        LOG_TAG,
                        "Failed to set toolbar title appearance to style resource id "
                                + titleAppearance,
                        e);
            }
        }
     ...
     */
    public static void setToolbarSubtitle(
            @NonNull AppCompatActivity activity,
            @IdRes int id,
            String subtitle,
            @StyleRes int subtitleAppearance) {
        Toolbar toolbar = activity.findViewById(id);
        if (toolbar != null) {
            ...
            try {
                ...
                Logger.logStackTraceWithMessage(
                        LOG_TAG,
                        "Failed to set toolbar subtitle appearance to style resource id "
                                + subtitleAppearance,
                        e);
            }
        }
     ...
     */
    public static void setShowBackButtonInActionBar(
            @NonNull AppCompatActivity activity, boolean showBackButtonInActionBar) {
        final ActionBar actionBar = activity.getSupportActionBar();
        if (actionBar != null) {
