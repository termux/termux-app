...
public class CrashHandler implements Thread.UncaughtExceptionHandler {
    ...
    public void uncaughtException(@NonNull Thread thread, @NonNull Throwable throwable) {
        Logger.logInfo(
                LOG_TAG, "uncaughtException() for " + thread + ": " + throwable.getMessage());
        logCrash(thread, throwable);

        // Don't stop the app if not on the main thread
        if (mIsDefaultHandler) mDefaultUEH.uncaughtException(thread, throwable);
    }
    ...
    public static void setDefaultCrashHandler(
        ...
        if (!(Thread.getDefaultUncaughtExceptionHandler() instanceof CrashHandler)) {
            Thread.setDefaultUncaughtExceptionHandler(
                    new CrashHandler(context, crashHandlerClient, true));
        }
    }
    ...
    public static void setCrashHandler(
        ...
        Thread.currentThread()
                .setUncaughtExceptionHandler(new CrashHandler(context, crashHandlerClient, false));
    }

    /**
     * Get {@link CrashHandler} instance that can be set as uncaught crash handler of a non-main
     * thread.
     */
    ...
    /**
     * Log a crash in the crash log file at path returned by {@link
        ...
        reportString
                ...
                .append(
                        Logger.getStackTracesMarkdownString(
                                "Stacktrace", Logger.getStackTracesStringArray(throwable)));
    ...
    public interface CrashHandlerClient {
        ...
        /**
         * Called before {@link #logCrashToFile(Context, CrashHandlerClient, Thread, Throwable)} is
         * called.
         *
         * @param context The {@link Context} passed to {@link CrashHandler#CrashHandler(Context,
         *     CrashHandlerClient, boolean)}.
         * @param thread The {@link Thread} in which the crash happened.
         * @param throwable The {@link Throwable} thrown for the crash.
        ...
        /**
         * Called after {@link #logCrashToFile(Context, CrashHandlerClient, Thread, Throwable)} is
         * called.
         *
         * @param context The {@link Context} passed to {@link CrashHandler#CrashHandler(Context,
         *     CrashHandlerClient, boolean)}.
