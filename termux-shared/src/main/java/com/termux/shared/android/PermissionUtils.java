...
public class PermissionUtils {
     ...
     * @return Returns {@code true} if requesting the permission was successful, otherwise {@code
     *     false}.
     */
    @RequiresApi(api = Build.VERSION_CODES.M)
    public static boolean requestPermission(
            @NonNull Context context, @NonNull String permission, int requestCode) {
     ...
     * Request user to grant required permissions to the app.
     *
     * <p>On sdk 30 (android 11), Activity.onRequestPermissionsResult() will pass {@link
     * PackageManager#PERMISSION_DENIED} (-1) without asking the user for the permission if user
     * previously denied the permission prompt. On sdk 29 (android 10),
     * Activity.onRequestPermissionsResult() will pass {@link PackageManager#PERMISSION_DENIED} (-1)
     * without asking the user for the permission if user previously selected "Deny & don't ask
     * again" option in prompt. The user will have to manually enable permission in app info in
     * Android settings. If user grants and then denies in settings, then next time prompt will
     * shown.
     *
     * @param context The context for operations. It must be an instance of {@link Activity} or
     *     {@link AppCompatActivity}.
     ...
     *     will fail silently and will log an exception.
     * @return Returns {@code true} if requesting the permissions was successful, otherwise {@code
     *     false}.
     */
    @RequiresApi(api = Build.VERSION_CODES.M)
    public static boolean requestPermissions(
            @NonNull Context context, @NonNull String[] permissions, int requestCode) {
        List<String> permissionsNotRequested = getPermissionsNotRequested(context, permissions);
        if (permissionsNotRequested.size() > 0) {
     ...
     * @param context The context for operations.
     * @param permissions The {@link String[]} names for permissions to check.
     * @return Returns {@link List<String>} of permissions that have not been requested. It will
     *     have size 0 if all permissions have been requested.
     */
    @NonNull
    public static List<String> getPermissionsNotRequested(
            @NonNull Context context, @NonNull String[] permissions) {
        List<String> permissionsNotRequested = new ArrayList<>();
        Collections.addAll(permissionsNotRequested, permissions);

        PackageInfo packageInfo =
                PackageUtils.getPackageInfoForPackage(context, PackageManager.GET_PERMISSIONS);
        if (packageInfo == null) {
        ...
        if (packageInfo.requestedPermissions == null
                || packageInfo.requestedPermissions.length == 0) return permissionsNotRequested;
    ...
    /**
     * If path is under primary external storage directory and storage permission is missing, then
     * legacy or manage external storage permission will be requested from the user via a call to
     * {@link #checkAndRequestLegacyOrManageExternalStoragePermission(Context, int, boolean)}.
     *
     * @param context The context for operations.
     * @param filePath The path to check.
     * @param requestCode The request code to use while asking for permission.
     * @param showErrorMessage If an error message toast should be shown if permission is not
     *     granted.
     * @return Returns {@code true} if permission is granted, otherwise {@code false}.
     */
    @SuppressLint("SdCardPath")
    public static boolean
            checkAndRequestLegacyOrManageExternalStoragePermissionIfPathOnPrimaryExternalStorage(
        ...
        return checkAndRequestLegacyOrManageExternalStoragePermission(
                context, requestCode, showErrorMessage);
    }

    /**
     * Check if legacy or manage external storage permissions has been granted. If {@link
     * #isLegacyExternalStoragePossible(Context)} returns {@code true}, them it will be checked if
     * app has has been granted {@link Manifest.permission#READ_EXTERNAL_STORAGE} and {@link
     * Manifest.permission#WRITE_EXTERNAL_STORAGE} permissions, otherwise it will be checked if app
     * has been granted the {@link Manifest.permission#MANAGE_EXTERNAL_STORAGE} permission.
     *
     * <p>If storage permission is missing, it will be requested from the user if {@code context} is
     * an instance of {@link Activity} or {@link AppCompatActivity} and {@code requestCode} is `>=0`
     * and the function will automatically return. The caller should register for
     ...
     *
     * <p>Caller must add following to AndroidManifest.xml of the app, otherwise errors will be
     * thrown. {@code <manifest <uses-permission
     ...
     *
     * <p><application android:requestLegacyExternalStorage="true" .... </application> </manifest> }
     *
     * @param context The context for operations.
     * @param requestCode The request code to use while asking for permission.
     * @param showErrorMessage If an error message toast should be shown if permission is not
     ...
     * @return Returns {@code true} if permission is granted, otherwise {@code false}.
     */
    public static boolean checkAndRequestLegacyOrManageExternalStoragePermission(
            @NonNull Context context, int requestCode, boolean showErrorMessage) {
        String errmsg;
        boolean requestLegacyStoragePermission = isLegacyExternalStoragePossible(context);
        boolean checkIfHasRequestedLegacyExternalStorage =
                checkIfHasRequestedLegacyExternalStorage(context);
        ...
        if (requestLegacyStoragePermission && checkIfHasRequestedLegacyExternalStorage) {
            ...
            if (!hasRequestedLegacyExternalStorage(context, showErrorMessage)) return false;
        }
        ...
        if (showErrorMessage) Logger.showToast(context, errmsg, false);

        if (requestCode < 0 || Build.VERSION.SDK_INT < Build.VERSION_CODES.M) return false;
     ...
     *     has been granted {@link Manifest.permission#READ_EXTERNAL_STORAGE} and {@link
     *     Manifest.permission#WRITE_EXTERNAL_STORAGE} permissions, otherwise it will be checked if
     *     app has been granted the {@link Manifest.permission#MANAGE_EXTERNAL_STORAGE} permission.
     * @return Returns {@code true} if permission is granted, otherwise {@code false}.
     */
    public static boolean checkStoragePermission(
            @NonNull Context context, boolean checkLegacyStoragePermission) {
    ...
    /**
     * Request user to grant {@link Manifest.permission#READ_EXTERNAL_STORAGE} and {@link
     ...
     */
    @RequiresApi(api = Build.VERSION_CODES.M)
    public static boolean requestLegacyStorageExternalPermission(
            @NonNull Context context, int requestCode) {
    ...
    /**
     * Request user to grant {@link Manifest.permission#MANAGE_EXTERNAL_STORAGE} permission to the
     * app.
     *
     * @param context The context for operations, like an {@link Activity} or {@link Service}
     *     context. It must be an instance of {@link Activity} or {@link AppCompatActivity} if
     *     result is required via the Activity#onActivityResult() callback and {@code requestCode}
     *     is `>=0`.
     * @param requestCode The request code to use while asking for permission. It must be `>=0` if
     *     result it required.
     * @return Returns the {@code error} if requesting the permission was not successful, otherwise
     *     {@code null}.
     */
    @RequiresApi(api = Build.VERSION_CODES.R)
    public static Error requestManageStorageExternalPermission(
            @NonNull Context context, int requestCode) {
        ...
        // Flag must not be passed for activity contexts, otherwise onActivityResult() will not be
        // called with permission grant result.
        // Flag must be passed for non-activity contexts like services, otherwise "Calling
        // startActivity() from outside of an Activity context requires the FLAG_ACTIVITY_NEW_TASK
        // flag" exception will be raised.
        ...
        else error = ActivityUtils.startActivity(context, intent, true, false);

        // Use fallback if matching Activity did not exist for
        ...
        if (error != null) {
            ...
            else return ActivityUtils.startActivity(context, intent);
        }
    ...
    /**
     * Return whether it should be checked if app has set {@link
     * android.R.attr#requestLegacyExternalStorage} attribute to {@code true}, if storage
     * permissions are to be requested based on if {@link #isLegacyExternalStoragePossible(Context)}
     * return {@code true}.
     *
     * <p>If app is targeting targetSdkVersion 30 (android 11), then legacy storage can only be
     * requested if running on sdk 29 (android 10). If app is targeting targetSdkVersion 29 (android
     * 10), then legacy storage can only be requested if running on sdk 29 (android 10) and higher.
     */
    ...
    /**
     * Call to {@link Environment#isExternalStorageLegacy()} will not return the actual value
     * defined in app manifest for {@link android.R.attr#requestLegacyExternalStorage} attribute,
     * since an app may inherit its legacy state based on when it was first installed, target sdk
     * and other factors. To provide consistent experience for all users regardless of current
     * legacy state on a specific device, we directly use the value defined in app` manifest.
     */
    public static boolean hasRequestedLegacyExternalStorage(
            @NonNull Context context, boolean showErrorMessage) {
        String errmsg;
        Boolean hasRequestedLegacyExternalStorage =
                PackageUtils.hasRequestedLegacyExternalStorage(context);
        if (hasRequestedLegacyExternalStorage != null && !hasRequestedLegacyExternalStorage) {
            ...
            Logger.logError(LOG_TAG, errmsg);
            if (showErrorMessage) Logger.showToast(context, errmsg, true);
            return false;
        }
     ...
     *     {@code null}.
     */
    public static Error requestDisplayOverOtherAppsPermission(
            @NonNull Context context, int requestCode) {
    ...
    /**
     * Check if running on sdk 29 (android 10) or higher and {@link
     * Manifest.permission#SYSTEM_ALERT_WINDOW} permission has been granted or not.
     *
     * @param context The context for operations.
     * @param logResults If it should be logged that permission has been granted or not.
     * @return Returns {@code true} if permission is granted, otherwise {@code false}.
     */
    public static boolean validateDisplayOverOtherAppsPermissionForPostAndroid10(
            @NonNull Context context, boolean logResults) {
        ...
        if (!checkDisplayOverOtherAppsPermission(context)) {
            if (logResults)
                Logger.logWarn(
                        LOG_TAG,
                        context.getPackageName()
                                + " does not have Display over other apps (SYSTEM_ALERT_WINDOW)"
                                + " permission");
            return false;
        } else {
            if (logResults)
                Logger.logDebug(
                        LOG_TAG,
                        context.getPackageName()
                                + " already has Display over other apps (SYSTEM_ALERT_WINDOW)"
                                + " permission");
            return true;
        }
    }
    ...
    public static Error requestDisableBatteryOptimizations(
            @NonNull Context context, int requestCode) {
