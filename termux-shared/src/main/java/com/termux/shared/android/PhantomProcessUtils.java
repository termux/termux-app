 ...
 *
 * <p>https://github.com/termux/termux-app/issues/2366
...
public class PhantomProcessUtils {
    ...
    /**
     * If feature flag set to false, then will disable trimming of phantom process and processes
     * using excessive CPU. Flag is available on Pixel Android 12L beta 3 and Android 13.
     * Availability on other devices will depend on if other vendors merged the 09dcdad5 commit or
     * not in their releases and if they actually want to support the flag. Check {@link
     * FeatureFlagUtils} javadocs for more details.
     */
    public static final String FEATURE_FLAG_SETTINGS_ENABLE_MONITOR_PHANTOM_PROCS =
            "settings_enable_monitor_phantom_procs";

    /**
     * Maximum number of allowed phantom processes. It is also used as the label for the currently
     * enforced ActivityManagerConstants MAX_PHANTOM_PROCESSES value in the `dumpsys activity
     * settings` output.
     *
     * <p>https://cs.android.com/android/platform/superproject/+/android-12.0.0_r4:frameworks/base/services/core/java/com/android/server/am/ActivityManagerConstants.java;l=574
     ...
     * is boolean (1 or 0). The value '1' means that
     * DeviceConfig#setProperties(DeviceConfig.Properties) will return {@code false}.
     *
     * <p>https://cs.android.com/android/platform/superproject/+/android-12.0.0_r4:frameworks/base/core/java/android/provider/DeviceConfig.java
     ...
     */
    public static final String SETTINGS_GLOBAL_DEVICE_CONFIG_SYNC_DISABLED =
     ...
     */
    @NonNull
    public static FeatureFlagUtils.FeatureFlagValue getFeatureFlagMonitorPhantomProcsValueString(
            @NonNull Context context) {
        return FeatureFlagUtils.getFeatureFlagValueString(
                context, FEATURE_FLAG_SETTINGS_ENABLE_MONITOR_PHANTOM_PROCS);
    }

    /**
     * Get currently enforced ActivityManagerConstants MAX_PHANTOM_PROCESSES value, defaults to 32.
     * Can be changed by modifying device config activity_manager namespace "max_phantom_processes"
     * value.
     *
     * @param context The {@link Context} for operations.
    ...
    public static Integer getActivityManagerMaxPhantomProcesses(@NonNull Context context) {
        ...
        // Dumpsys logs the currently enforced MAX_PHANTOM_PROCESSES value and not the device config
        // setting.
        String script =
                "/system/bin/dumpsys activity settings | /system/bin/grep -iE '^[\t ]+"
                        + KEY_MAX_PHANTOM_PROCESSES
                        + "=[0-9]+$' | /system/bin/cut -d = -f2";
        ...
        executionCommand.commandLabel =
                " ActivityManager " + KEY_MAX_PHANTOM_PROCESSES + " Command";
        executionCommand.backgroundCustomLogLevel = Logger.LOG_LEVEL_OFF;
        AppShell appShell =
                AppShell.execute(
                        context, executionCommand, null, new AndroidShellEnvironment(), null, true);
    ...
    public static Integer getSettingsGlobalDeviceConfigSyncDisabled(@NonNull Context context) {
        return (Integer)
                SettingsProviderUtils.getSettingsValue(
                        context,
