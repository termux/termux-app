...
/**
 * A class that maintains info for foreground Termux sessions. It also provides a way to link each
 * {@link TerminalSession} with the {@link ExecutionCommand} that started it.
 */
public class TermuxSession {
    ...
    /**
     * Start execution of an {@link ExecutionCommand} with {@link Runtime#exec(String[], String[],
     * File)}.
     *
     * <p>The {@link ExecutionCommand#executable}, must be set, {@link
     * ExecutionCommand#commandLabel}, {@link ExecutionCommand#arguments} and {@link
     * ExecutionCommand#workingDirectory} may optionally be set.
     *
     * <p>If {@link ExecutionCommand#executable} is {@code null}, then a default shell is
     * automatically chosen.
     *
     * @param currentPackageContext The {@link Context} for operations. This must be the context for
     *     the current package and not the context of a `sharedUserId` package, since environment
     *     setup may be dependent on current package.
     * @param executionCommand The {@link ExecutionCommand} containing the information for execution
     *     command.
     ...
     * @param setStdoutOnExit If set to {@code true}, then the {@link ResultData#stdout} available
     *     in the {@link TermuxSessionClient#onTermuxSessionExited(TermuxSession)} callback will be
     *     set to the {@link TerminalSession} transcript. The session transcript will contain both
     *     stdout and stderr combined, basically anything sent to the the pseudo terminal /dev/pts,
     *     including PS1 prefixes. Set this to {@code true} only if the session transcript is
     *     required, since this requires extra processing to get it.
     * @return Returns the {@link TermuxSession}. This will be {@code null} if failed to start the
     *     execution command.
     */
    public static TermuxSession execute(
            @NonNull final Context currentPackageContext,
        ...
        if (executionCommand.workingDirectory == null
                || executionCommand.workingDirectory.isEmpty())
        ...
        if (executionCommand.executable == null) {
            ...
            if (executionCommand.executable == null) {
                ...
                // Do not start a login shell since ~/.profile may cause startup failure if its
                // invalid.
                // /system/bin/sh is provided by mksh (not toybox) and does load .mkshrc but for
                // android its set
        ...
        // Setup command args
        String[] commandArgs =
                shellEnvironmentClient.setupShellCommandArguments(
        ...
        List<String> environmentList =
                ShellEnvironmentUtils.convertEnvironmentToEnviron(environment);
        ...
        return new TermuxSession(
                terminalSession, executionCommand, termuxSessionClient, setStdoutOnExit);
    }

    /**
     * Signal that this {@link TermuxSession} has finished. This should be called when {@link
     * TerminalSessionClient#onSessionFinished(TerminalSession)} callback is received by the caller.
     *
     * <p>If the processes has finished, then sets {@link ResultData#stdout}, {@link
     * ResultData#stderr} and {@link ResultData#exitCode} for the {@link #mExecutionCommand} of the
     * {@code termuxTask} and then calls {@link #processTermuxSessionResult(TermuxSession,
     * ExecutionCommand)} to process the result}.
     */
    public void finish() {
        ...
        if (exitCode == 0)
            Logger.logDebug(
                    ...
                    "The \""
                            ...
                            + "\" TermuxSession exited normally");
        else
            Logger.logDebug(
                    LOG_TAG,
                    "The \""
                            + mExecutionCommand.getCommandIdAndLabelLogString()
                            + "\" TermuxSession exited with code: "
                            + exitCode);
        ...
        if (mExecutionCommand.isStateFailed()) {
            Logger.logDebug(
                    ...
                    "Ignoring setting \""
                            + mExecutionCommand.getCommandIdAndLabelLogString()
                            + "\" TermuxSession state to ExecutionState.EXECUTED and processing"
                            + " results since it has already failed");
            return;
        }
        ...
        if (this.mSetStdoutOnExit)
            mExecutionCommand.resultData.stdout.append(
                    ShellUtils.getTerminalSessionTranscriptText(mTerminalSession, true, false));
    ...
    /**
     * Kill this {@link TermuxSession} by sending a {@link OsConstants#SIGILL} to its {@link
     * #mTerminalSession} if its still executing.
     *
     * @param context The {@link Context} for operations.
     * @param processResult If set to {@code true}, then the {@link
     *     #processTermuxSessionResult(TermuxSession, ExecutionCommand)} will be called to process
     *     the failure.
     */
    public void killIfExecuting(@NonNull final Context context, boolean processResult) {
        // If execution command has already finished executing, then no need to process results or
        // send SIGKILL
        if (mExecutionCommand.hasExecuted()) {
            Logger.logDebug(
                    LOG_TAG,
                    "Ignoring sending SIGKILL to \""
                            + mExecutionCommand.getCommandIdAndLabelLogString()
     ...
     *
     * <p>Only one of {@code termuxSession} and {@code executionCommand} must be set.
     *
     * <p>If the {@code termuxSession} and its {@link #mTermuxSessionClient} are not {@code null},
     * then the {@link TermuxSession.TermuxSessionClient#onTermuxSessionExited(TermuxSession)}
     * callback will be called.
     *
     * @param termuxSession The {@link TermuxSession}, which should be set if {@link
     *     #execute(Context, ExecutionCommand, TerminalSessionClient, TermuxSessionClient,
     *     IShellEnvironment, HashMap, boolean)} successfully started the process.
     * @param executionCommand The {@link ExecutionCommand}, which should be set if {@link
     *     #execute(Context, ExecutionCommand, TerminalSessionClient, TermuxSessionClient,
     *     IShellEnvironment, HashMap, boolean)} failed to start the process.
     */
    private static void processTermuxSessionResult(
            final TermuxSession termuxSession, ExecutionCommand executionCommand) {
        ...
        if (executionCommand.shouldNotProcessResults()) {
            Logger.logDebug(
                    LOG_TAG,
                    "Ignoring duplicate call to process \""
                            + executionCommand.getCommandIdAndLabelLogString()
                            + "\" TermuxSession result");
            return;
        }
        ...
        if (termuxSession != null && termuxSession.mTermuxSessionClient != null) {
            ...
            // If a callback is not set and execution command didn't fail, then we set success state
            // now
