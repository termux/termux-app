...
public class TermuxPluginUtils {
     ...
     *
     * <p>The ExecutionCommand currentState must be greater or equal to {@link
     * ExecutionCommand.ExecutionState#EXECUTED}. If the {@link
     * ExecutionCommand#isPluginExecutionCommand} is {@code true} and {@link
     * ResultConfig#resultPendingIntent} or {@link ResultConfig#resultDirectoryPath} is not {@code
     * null}, then the result of commands are sent back to the command caller.
     *
     * @param context The {@link Context} that will be used to send result intent to the {@link
     *     PendingIntent} creator.
     * @param logTag The log tag to use for logging.
     * @param executionCommand The {@link ExecutionCommand} to process.
     */
    public static void processPluginExecutionCommandResult(
            final Context context, String logTag, final ExecutionCommand executionCommand) {
        ...
        if (!executionCommand.hasExecuted()) {
            Logger.logWarn(
                    ...
                    executionCommand.getCommandIdAndLabelLogString()
                            + ": Ignoring call to processPluginExecutionCommandResult() since the"
                            + " execution command state is not higher than the"
                            + " ExecutionState.EXECUTED");
            return;
        }

        boolean isPluginExecutionCommandWithPendingResult =
        ...
        // Log the output. ResultData should not be logged if pending result since ResultSender will
        // do it
        // or if logging is disabled
        Logger.logDebugExtended(
                logTag,
                ExecutionCommand.getExecutionOutputLogString(
                        executionCommand,
                        true,
        ...
        if (isPluginExecutionCommandWithPendingResult) {
            ...
            error =
                    ResultSender.sendCommandResultData(
                            ...
                            logTag,
                            executionCommand.getCommandIdAndLabelLogString(),
                            executionCommand.resultConfig,
                            executionCommand.resultData,
                            isExecutionCommandLoggingEnabled);
            if (error != null) {
                ...
                Logger.logDebugExtended(
                        logTag,
                        ExecutionCommand.getExecutionOutputLogString(
                                executionCommand, true, true, isExecutionCommandLoggingEnabled));

                // Flash and send notification for the error
                sendPluginCommandErrorNotification(
                        context,
                        logTag,
                        null,
                        ...
                        false,
                        true,
                        TermuxUtils.AppInfoMode.TERMUX_AND_CALLING_PACKAGE,
                        true,
     ...
     * process error with {@link #processPluginExecutionCommandError(Context, String,
     * ExecutionCommand, boolean)}.
     *
     ...
     *     regardless of if pending intent is {@code null} or {@link
     *     TERMUX_APP#KEY_PLUGIN_ERROR_NOTIFICATIONS_ENABLED} is {@code false}.
     * @param errmsg The error message to set.
     */
    public static void setAndProcessPluginExecutionCommandError(
     ...
     *
     * <p>The ExecutionCommand currentState must be equal to {@link
     * ExecutionCommand.ExecutionState#FAILED}. The {@link ResultData#getErrCode()} must have been
     * set to a value greater than {@link Errno#ERRNO_SUCCESS}. The {@link ResultData#errorsList}
     * must also be set with appropriate error info.
     *
     * <p>If the {@link ExecutionCommand#isPluginExecutionCommand} is {@code true} and {@link
     * ResultConfig#resultPendingIntent} or {@link ResultConfig#resultDirectoryPath} is not {@code
     * null}, then the errors of commands are sent back to the command caller.
     *
     * <p>Otherwise if the {@link TERMUX_APP#KEY_PLUGIN_ERROR_NOTIFICATIONS_ENABLED} is enabled,
     * then a flash and a notification will be shown for the error as well on the {@link
     * TermuxConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME} channel instead of
    ...
    public static void processPluginExecutionCommandError(
        ...
        if (!executionCommand.isStateFailed()) {
            Logger.logWarn(
                    logTag,
                    executionCommand.getCommandIdAndLabelLogString()
                            + ": Ignoring call to processPluginExecutionCommandError() since the"
                            + " execution command is not in ExecutionState.FAILED");
            return;
        }
        ...
        // Log the error and any exception. ResultData should not be logged if pending result since
        // ResultSender will do it
        Logger.logError(
                logTag,
        ...
        if (isPluginExecutionCommandWithPendingResult) {
            ...
            // No need to show notifications if a pending intent was sent, let the caller handle the
            // result himself
    ...
    /**
     * Set variables which will be used by {@link ResultSender#sendCommandResultData(Context,
     * String, String, ResultConfig, ResultData, boolean)} to send back the result via {@link
     * ResultConfig#resultPendingIntent}.
     */
    public static void setPluginResultPendingIntentVariables(ExecutionCommand executionCommand) {
        ...
        resultConfig.resultStdoutOriginalLengthKey =
        ...
        resultConfig.resultStderrOriginalLengthKey =
    ...
    /**
     * Set variables which will be used by {@link ResultSender#sendCommandResultData(Context,
     * String, String, ResultConfig, ResultData, boolean)} to send back the result by writing it to
     * files in {@link ResultConfig#resultDirectoryPath}.
     */
    public static void setPluginResultDirectoryVariables(ExecutionCommand executionCommand) {
        ResultConfig resultConfig = executionCommand.resultConfig;

        resultConfig.resultDirectoryPath =
                TermuxFileUtils.getCanonicalPath(resultConfig.resultDirectoryPath, null, true);
        resultConfig.resultDirectoryAllowedParentPath =
                TermuxFileUtils.getMatchedAllowedTermuxWorkingDirectoryParentPathForPath(
                        resultConfig.resultDirectoryPath);

        // Set default resultFileBasename if resultSingleFile is true to
        // `<executable_basename>-<timestamp>.log`
    ...
    /**
     * Send a plugin error report notification for {@link
     * TermuxConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID} and {@link
     ...
     */
    public static void sendPluginCommandErrorNotification(
        ...
        sendPluginCommandErrorNotification(
                ...
                MarkdownUtils.getMarkdownCodeForString(
                        Logger.getMessageAndStackTraceString(message, throwable), true),
                false,
                false,
                true);
    }
     ...
     * @param notificationTextString The text of the notification.
     * @param message The message for the error report.
     * @param forceNotification If set to {@code true}, then a notification will be shown regardless
     *     of if pending intent is {@code null} or {@link
     *     TermuxPreferenceConstants.TERMUX_APP#KEY_PLUGIN_ERROR_NOTIFICATIONS_ENABLED} is {@code
     *     false}.
     * @param showToast If set to {@code true}, then a toast will be shown for {@code
     *     notificationTextString}.
     * @param addDeviceInfo If set to {@code true}, then device info should be appended to the
     *     message.
     */
    public static void sendPluginCommandErrorNotification(
            final Context currentPackageContext,
        ...
        sendPluginCommandErrorNotification(
                ...
                forceNotification,
                showToast,
                TermuxUtils.AppInfoMode.TERMUX_AND_PLUGIN_PACKAGE,
                addDeviceInfo,
                null);
    }

    /**
     * Send a plugin error notification for {@link
     * TermuxConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID} and {@link
     * TermuxConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME}.
     *
     * @param currentPackageContext The {@link Context} of current package.
    ...
    public static void sendPluginCommandErrorNotification(
        ...
        // Note: Do not change currentPackageContext or termuxPackageContext passed to functions or
        // things will break
        ...
        TermuxAppSharedPreferences preferences =
                TermuxAppSharedPreferences.build(termuxPackageContext);
        if (preferences == null) return;

        // If user has disabled notifications for plugin commands, then just return
        if (!preferences.arePluginErrorNotificationsEnabled(true) && !forceNotification) return;

        logTag = DataUtils.getDefaultIfNull(logTag, LOG_TAG);

        if (showToast) Logger.showToast(currentPackageContext, notificationTextString, true);
        ...
        ReportActivity.NewInstanceResult result =
                ReportActivity.newInstance(termuxPackageContext, reportInfo);
        if (result.contentIntent == null) return;

        // Must ensure result code for PendingIntents and id for notification are unique otherwise
        // will override previous
        int nextNotificationId =
                TermuxNotificationUtils.getNextNotificationId(termuxPackageContext);
        ...
        Notification.Builder builder =
                getPluginCommandErrorsNotificationBuilder(
                        currentPackageContext,
                        termuxPackageContext,
                        title,
                        notificationTextCharSequence,
                        notificationTextCharSequence,
                        contentIntent,
                        deleteIntent,
        ...
        NotificationManager notificationManager =
                NotificationUtils.getNotificationManager(termuxPackageContext);
        if (notificationManager != null)
            notificationManager.notify(nextNotificationId, builder.build());
    }

    /**
     * Get {@link Notification.Builder} for {@link
     * TermuxConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID} and {@link
     * TermuxConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME}.
     *
     * @param currentPackageContext The {@link Context} of current package.
     ...
     * @param contentIntent The {@link PendingIntent} which should be sent when notification is
     *     clicked.
     * @param deleteIntent The {@link PendingIntent} which should be sent when notification is
     *     deleted.
     * @param notificationMode The notification mode. It must be one of {@code
     *     NotificationUtils.NOTIFICATION_MODE_*}.
    ...
    /**
     * Setup the notification channel for {@link
     * TermuxConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID} and {@link
     * TermuxConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME}.
     *
     * @param context The {@link Context} for operations.
     ...
     */
    public static String checkIfAllowExternalAppsPolicyIsViolated(
            final Context context, String apiName) {
        String errmsg = null;
        ...
        if (mProperties == null || !mProperties.shouldAllowExternalApps()) {
            errmsg =
                    context.getString(
                            R.string.error_allow_external_apps_ungranted,
                            apiName,
                            TermuxFileUtils.getUnExpandedTermuxPath(
