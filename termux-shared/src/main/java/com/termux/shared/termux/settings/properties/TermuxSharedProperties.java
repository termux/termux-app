...
public abstract class TermuxSharedProperties {
    ...
    public synchronized void loadTermuxPropertiesFromDisk() {
        ...
        // was found, then default props would keep loading, since mSharedProperties would be null.
        // #2836
        ...
        mSharedProperties =
                new SharedProperties(
                        mContext, mPropertiesFile, mPropertiesList, mSharedPropertiesParser);
     ...
     *     Otherwise the {@link Properties} object is read directly from the {@link
     ...
     * @param cached If {@code true}, then the value is returned from the the {@link Properties}
     *     in-memory cache. Otherwise the {@link Properties} object is read directly from the file
    ...
    /**
     * A function to check if the value is {@code true} for {@link Properties} key read from the
     * {@link #mPropertiesFile} file.
     *
     * @param key The key to read.
     * @param cached If {@code true}, then the value is checked from the the {@link Properties}
     *     in-memory cache. Otherwise the {@link Properties} object is read directly from the file
     *     and value is checked from it.
     * @param logErrorOnInvalidValue If {@code true}, then an error will be logged if key value was
     *     found in {@link Properties} but was invalid.
     * @return Returns the {@code true} if the {@link Properties} key {@link String} value equals
     *     "true", regardless of case. If the key does not exist in the file or does not equal
     *     "true", then {@code false} will be returned.
     */
    public boolean isPropertyValueTrue(String key, boolean cached, boolean logErrorOnInvalidValue) {
        return (boolean)
                SharedProperties.getBooleanValueForStringValue(
                        key,
                        (String) getPropertyValue(key, null, cached),
                        false,
    ...
    /**
     * A function to check if the value is {@code false} for {@link Properties} key read from the
     * {@link #mPropertiesFile} file.
     *
     * @param key The key to read.
     * @param cached If {@code true}, then the value is checked from the the {@link Properties}
     ...
     * @param logErrorOnInvalidValue If {@code true}, then an error will be logged if key value was
     *     found in {@link Properties} but was invalid.
     * @return Returns {@code true} if the {@link Properties} key {@link String} value equals
     *     "false", regardless of case. If the key does not exist in the file or does not equal
     *     "false", then {@code true} will be returned.
     */
    public boolean isPropertyValueFalse(
            String key, boolean cached, boolean logErrorOnInvalidValue) {
        return (boolean)
                SharedProperties.getInvertedBooleanValueForStringValue(
                        key,
                        (String) getPropertyValue(key, null, cached),
                        true,
                        logErrorOnInvalidValue,
    ...
    /**
     * Get the internal value {@link Object} {@link HashMap <>} in-memory cache for the {@link
     * #mPropertiesFile} file. A call to {@link #loadTermuxPropertiesFromDisk()} must be made before
     * this.
     *
     ...
     * file. If cache is {@code true}, then value is returned from the {@link HashMap <>} in-memory
     * cache, so a call to {@link #loadTermuxPropertiesFromDisk()} must be made before this.
     *
     * @param key The key to read from the {@link HashMap<>} in-memory cache.
     * @param cached If {@code true}, then the value is returned from the the {@link HashMap <>}
     *     in-memory cache, but if the value is null, then an attempt is made to return the default
     *     value. If {@code false}, then the {@link Properties} object is read directly from the
     *     file and internal value is returned for the property value against the key.
        ...
        if (cached) {
            ...
            if (value != null || mSharedProperties.getInternalProperties().containsKey(key)) {
                ...
                Logger.logWarn(
                        ...
                        "The value for \""
                                + key
                                + "\" not found in SharedProperties cache, force returning default"
                                + " value: `"
                                + value
                                + "`");
                return value;
            ...
            return getInternalTermuxPropertyValueFromValue(
                    mContext, key, mSharedProperties.getProperty(key, false));
        }
    }

    /**
     * Get the internal {@link Object} value for the key passed from the first file found in {@link
     * TermuxConstants#TERMUX_PROPERTIES_FILE_PATHS_LIST}. The {@link Properties} object is read
     * directly from the file and internal value is returned for the property value against the key.
     *
    ...
    public static Object getTermuxInternalPropertyValue(Context context, String key) {
        return SharedProperties.getInternalProperty(
                context,
                SharedProperties.getPropertiesFileFromList(
    ...
    public static class SharedPropertiesParserClient implements SharedPropertiesParser {
        @NonNull
        @Override
        public Properties preProcessPropertiesOnReadFromDisk(
                @NonNull Context context, @NonNull Properties properties) {
            return replaceUseBlackUIProperty(properties);
        }

        /**
         * Override the {@link
         * SharedPropertiesParser#getInternalPropertyValueFromValue(Context,String,String)}
         * interface function.
         */
        @Override
        public Object getInternalPropertyValueFromValue(
                @NonNull Context context, String key, String value) {
    ...
    public static Properties replaceUseBlackUIProperty(@NonNull Properties properties) {
        ...
        Logger.logWarn(
                ...
                "Removing deprecated property "
                        + TermuxPropertyConstants.KEY_USE_BLACK_UI
                        + "="
        ...
        if (properties.getProperty(TermuxPropertyConstants.KEY_NIGHT_MODE) == null) {
            Boolean useBlackUI =
                    SharedProperties.getBooleanValueForStringValue(useBlackUIStringValue);
            if (useBlackUI != null) {
                String termuxAppTheme =
                        useBlackUI
     ...
     */
    public static Object getInternalTermuxPropertyValueFromValue(
        ...
        */
        switch (key) {
            ...
            case TermuxPropertyConstants.KEY_DELETE_TMPDIR_FILES_OLDER_THAN_X_DAYS_ON_EXIT:
                return (int)
                        getDeleteTMPDIRFilesOlderThanXDaysOnExitInternalPropertyValueFromValue(
                                value);
            ...
            default:
                ...
                if (TermuxPropertyConstants.TERMUX_DEFAULT_FALSE_BOOLEAN_BEHAVIOUR_PROPERTIES_LIST
                        .contains(key))
                    return (boolean)
                            SharedProperties.getBooleanValueForStringValue(
                                    key, value, false, true, LOG_TAG);
                // default true boolean behaviour
                if (TermuxPropertyConstants.TERMUX_DEFAULT_TRUE_BOOLEAN_BEHAVIOUR_PROPERTIES_LIST
                        .contains(key))
                    return (boolean)
                            SharedProperties.getBooleanValueForStringValue(
                                    key, value, true, true, LOG_TAG);
                // default inverted false boolean behaviour
                // else if
                // (TermuxPropertyConstants.TERMUX_DEFAULT_INVERETED_FALSE_BOOLEAN_BEHAVIOUR_PROPERTIES_LIST.contains(key))
                //    return (boolean) SharedProperties.getInvertedBooleanValueForStringValue(key,
                // value, false, true, LOG_TAG);
                // default inverted true boolean behaviour
                // else if
                // (TermuxPropertyConstants.TERMUX_DEFAULT_INVERETED_TRUE_BOOLEAN_BEHAVIOUR_PROPERTIES_LIST.contains(key))
                //    return (boolean) SharedProperties.getInvertedBooleanValueForStringValue(key,
                // value, true, true, LOG_TAG);
     ...
     * TermuxPropertyConstants#MAP_BELL_BEHAVIOUR} if the value is not {@code null} and is valid,
     * otherwise returns {@link TermuxPropertyConstants#DEFAULT_IVALUE_BELL_BEHAVIOUR}.
     *
    ...
    public static int getBellBehaviourInternalPropertyValueFromValue(String value) {
        return (int)
                SharedProperties.getDefaultIfNotInMap(
    ...
    /**
     * Returns the int for the value if its not null and is between {@link
     * TermuxPropertyConstants#IVALUE_DELETE_TMPDIR_FILES_OLDER_THAN_X_DAYS_ON_EXIT_MIN} and {@link
     * TermuxPropertyConstants#IVALUE_DELETE_TMPDIR_FILES_OLDER_THAN_X_DAYS_ON_EXIT_MAX}, otherwise
     * returns {@link
     ...
     * @return Returns the internal value for value.
     */
    public static int getDeleteTMPDIRFilesOlderThanXDaysOnExitInternalPropertyValueFromValue(
            String value) {
        return SharedProperties.getDefaultIfNotInRange(
                ...
                DataUtils.getIntFromString(
                        ...
                        TermuxPropertyConstants
                                .DEFAULT_IVALUE_DELETE_TMPDIR_FILES_OLDER_THAN_X_DAYS_ON_EXIT),
                TermuxPropertyConstants
                        .DEFAULT_IVALUE_DELETE_TMPDIR_FILES_OLDER_THAN_X_DAYS_ON_EXIT,
    ...
    /**
     * Returns the int for the value if its not null and is between {@link
     * TermuxPropertyConstants#IVALUE_TERMINAL_CURSOR_BLINK_RATE_MIN} and {@link
     * TermuxPropertyConstants#IVALUE_TERMINAL_CURSOR_BLINK_RATE_MAX}, otherwise returns {@link
    ...
    public static int getTerminalCursorBlinkRateInternalPropertyValueFromValue(String value) {
        return SharedProperties.getDefaultIfNotInRange(
                ...
                DataUtils.getIntFromString(
                        value, TermuxPropertyConstants.DEFAULT_IVALUE_TERMINAL_CURSOR_BLINK_RATE),
                TermuxPropertyConstants.DEFAULT_IVALUE_TERMINAL_CURSOR_BLINK_RATE,
    ...
    /**
     * Returns the internal value after mapping it based on {@link
     * TermuxPropertyConstants#MAP_TERMINAL_CURSOR_STYLE} if the value is not {@code null} and is
     * valid, otherwise returns {@link
    ...
    /**
     * Returns the int for the value if its not null and is between {@link
     * TermuxPropertyConstants#IVALUE_TERMINAL_MARGIN_HORIZONTAL_MIN} and {@link
     * TermuxPropertyConstants#IVALUE_TERMINAL_MARGIN_HORIZONTAL_MAX}, otherwise returns {@link
    ...
    public static int getTerminalMarginHorizontalInternalPropertyValueFromValue(String value) {
        return SharedProperties.getDefaultIfNotInRange(
                ...
                DataUtils.getIntFromString(
                        value, TermuxPropertyConstants.DEFAULT_IVALUE_TERMINAL_MARGIN_HORIZONTAL),
                TermuxPropertyConstants.DEFAULT_IVALUE_TERMINAL_MARGIN_HORIZONTAL,
    ...
    /**
     * Returns the int for the value if its not null and is between {@link
     * TermuxPropertyConstants#IVALUE_TERMINAL_MARGIN_VERTICAL_MIN} and {@link
     * TermuxPropertyConstants#IVALUE_TERMINAL_MARGIN_VERTICAL_MAX}, otherwise returns {@link
    ...
    public static int getTerminalMarginVerticalInternalPropertyValueFromValue(String value) {
        return SharedProperties.getDefaultIfNotInRange(
                ...
                DataUtils.getIntFromString(
                        value, TermuxPropertyConstants.DEFAULT_IVALUE_TERMINAL_MARGIN_VERTICAL),
                TermuxPropertyConstants.DEFAULT_IVALUE_TERMINAL_MARGIN_VERTICAL,
    ...
    /**
     * Returns the int for the value if its not null and is between {@link
     * TermuxPropertyConstants#IVALUE_TERMINAL_TRANSCRIPT_ROWS_MIN} and {@link
     * TermuxPropertyConstants#IVALUE_TERMINAL_TRANSCRIPT_ROWS_MAX}, otherwise returns {@link
    ...
    public static int getTerminalTranscriptRowsInternalPropertyValueFromValue(String value) {
        return SharedProperties.getDefaultIfNotInRange(
                ...
                DataUtils.getIntFromString(
                        value, TermuxPropertyConstants.DEFAULT_IVALUE_TERMINAL_TRANSCRIPT_ROWS),
                TermuxPropertyConstants.DEFAULT_IVALUE_TERMINAL_TRANSCRIPT_ROWS,
    ...
    /**
     * Returns the int for the value if its not null and is between {@link
     * TermuxPropertyConstants#IVALUE_TERMINAL_TOOLBAR_HEIGHT_SCALE_FACTOR_MIN} and {@link
     * TermuxPropertyConstants#IVALUE_TERMINAL_TOOLBAR_HEIGHT_SCALE_FACTOR_MAX}, otherwise returns
     * {@link TermuxPropertyConstants#DEFAULT_IVALUE_TERMINAL_TOOLBAR_HEIGHT_SCALE_FACTOR}.
     *
     * @param value The {@link String} value to convert.
     * @return Returns the internal value for value.
     */
    public static float getTerminalToolbarHeightScaleFactorInternalPropertyValueFromValue(
        ...
        return SharedProperties.getDefaultIfNotInRange(
                ...
                DataUtils.getFloatFromString(
                        value,
                        TermuxPropertyConstants
                                .DEFAULT_IVALUE_TERMINAL_TOOLBAR_HEIGHT_SCALE_FACTOR),
    ...
    /**
     * Returns the code point for the value if key is not {@code null} and value is not {@code null}
     * and is valid, otherwise returns {@code null}.
     *
    ...
    public static Integer getCodePointForSessionShortcuts(String key, String value) {
        ...
        if (!(parts.length == 2 && parts[0].trim().equals("ctrl"))
                || input.isEmpty()
                || input.length() > 2) {
    ...
    /**
     * Returns the value itself if it is not {@code null}, otherwise returns {@link
    ...
    /**
     * Returns the path itself if a directory exists at it and is readable, otherwise returns {@link
        ...
        if (!workDir.exists() || !workDir.isDirectory() || !workDir.canRead()) {
            ...
            Logger.logError(
                    ...
                    "The path \""
                            ...
                            + "\" does not exist, is not a directory or is not readable. Using"
                            + " default value \""
                            + TermuxPropertyConstants.DEFAULT_IVALUE_DEFAULT_WORKING_DIRECTORY
                            + "\" instead.");
            return TermuxPropertyConstants.DEFAULT_IVALUE_DEFAULT_WORKING_DIRECTORY;
    ...
    public static String getExtraKeysInternalPropertyValueFromValue(String value) {
        return SharedProperties.getDefaultIfNullOrEmpty(
                value, TermuxPropertyConstants.DEFAULT_IVALUE_EXTRA_KEYS);
    }
    ...
    public static String getExtraKeysStyleInternalPropertyValueFromValue(String value) {
        return SharedProperties.getDefaultIfNullOrEmpty(
                value, TermuxPropertyConstants.DEFAULT_IVALUE_EXTRA_KEYS_STYLE);
    }
    ...
    public boolean isFileShareReceiverDisabled() {
        return (boolean)
                getInternalPropertyValue(
                        TermuxPropertyConstants.KEY_DISABLE_FILE_SHARE_RECEIVER, true);
    }

    public boolean isFileViewReceiverDisabled() {
        return (boolean)
                getInternalPropertyValue(
                        TermuxPropertyConstants.KEY_DISABLE_FILE_VIEW_RECEIVER, true);
    }

    public boolean areHardwareKeyboardShortcutsDisabled() {
        return (boolean)
                getInternalPropertyValue(
                        TermuxPropertyConstants.KEY_DISABLE_HARDWARE_KEYBOARD_SHORTCUTS, true);
    }

    public boolean areTerminalSessionChangeToastsDisabled() {
        return (boolean)
                getInternalPropertyValue(
                        TermuxPropertyConstants.KEY_DISABLE_TERMINAL_SESSION_CHANGE_TOAST, true);
    }

    public boolean isEnforcingCharBasedInput() {
        return (boolean)
                getInternalPropertyValue(
                        TermuxPropertyConstants.KEY_ENFORCE_CHAR_BASED_INPUT, true);
    }

    public boolean shouldExtraKeysTextBeAllCaps() {
        return (boolean)
                getInternalPropertyValue(
                        TermuxPropertyConstants.KEY_EXTRA_KEYS_TEXT_ALL_CAPS, true);
    }

    public boolean shouldSoftKeyboardBeHiddenOnStartup() {
        return (boolean)
                getInternalPropertyValue(
                        TermuxPropertyConstants.KEY_HIDE_SOFT_KEYBOARD_ON_STARTUP, true);
    }
    ...
    public boolean shouldOpenTerminalTranscriptURLOnClick() {
        return (boolean)
                getInternalPropertyValue(
                        TermuxPropertyConstants.KEY_TERMINAL_ONCLICK_URL_OPEN, true);
    }

    public boolean isUsingCtrlSpaceWorkaround() {
        return (boolean)
                getInternalPropertyValue(
                        TermuxPropertyConstants.KEY_USE_CTRL_SPACE_WORKAROUND, true);
    }
    ...
    public boolean isUsingFullScreenWorkAround() {
        return (boolean)
                getInternalPropertyValue(
                        TermuxPropertyConstants.KEY_USE_FULLSCREEN_WORKAROUND, true);
    }
    ...
    public int getTerminalCursorBlinkRate() {
        return (int)
                getInternalPropertyValue(
                        TermuxPropertyConstants.KEY_TERMINAL_CURSOR_BLINK_RATE, true);
    }

    public int getTerminalCursorStyle() {
        return (int)
                getInternalPropertyValue(TermuxPropertyConstants.KEY_TERMINAL_CURSOR_STYLE, true);
    }

    public int getTerminalMarginHorizontal() {
        return (int)
                getInternalPropertyValue(
                        TermuxPropertyConstants.KEY_TERMINAL_MARGIN_HORIZONTAL, true);
    }

    public int getTerminalMarginVertical() {
        return (int)
                getInternalPropertyValue(
                        TermuxPropertyConstants.KEY_TERMINAL_MARGIN_VERTICAL, true);
    }

    public int getTerminalTranscriptRows() {
        return (int)
                getInternalPropertyValue(
                        TermuxPropertyConstants.KEY_TERMINAL_TRANSCRIPT_ROWS, true);
    }

    public float getTerminalToolbarHeightScaleFactor() {
        return (float)
                getInternalPropertyValue(
                        TermuxPropertyConstants.KEY_TERMINAL_TOOLBAR_HEIGHT_SCALE_FACTOR, true);
    }

    public boolean isBackKeyTheEscapeKey() {
        return (boolean)
                TermuxPropertyConstants.IVALUE_BACK_KEY_BEHAVIOUR_ESCAPE.equals(
                        getInternalPropertyValue(
                                TermuxPropertyConstants.KEY_BACK_KEY_BEHAVIOUR, true));
    }

    public String getDefaultWorkingDirectory() {
        return (String)
                getInternalPropertyValue(
                        TermuxPropertyConstants.KEY_DEFAULT_WORKING_DIRECTORY, true);
    }
    ...
    /**
     * Get the {@link TermuxPropertyConstants#KEY_NIGHT_MODE} value from the properties file on
     * disk.
     */
    public static String getNightMode(Context context) {
        return (String)
                TermuxSharedProperties.getTermuxInternalPropertyValue(
                        context, TermuxPropertyConstants.KEY_NIGHT_MODE);
    }

    public boolean shouldEnableDisableSoftKeyboardOnToggle() {
        return (boolean)
                TermuxPropertyConstants.IVALUE_SOFT_KEYBOARD_TOGGLE_BEHAVIOUR_ENABLE_DISABLE.equals(
                        getInternalPropertyValue(
                                TermuxPropertyConstants.KEY_SOFT_KEYBOARD_TOGGLE_BEHAVIOUR, true));
    }

    public boolean areVirtualVolumeKeysDisabled() {
        return (boolean)
                TermuxPropertyConstants.IVALUE_VOLUME_KEY_BEHAVIOUR_VOLUME.equals(
                        getInternalPropertyValue(
                                TermuxPropertyConstants.KEY_VOLUME_KEYS_BEHAVIOUR, true));
    }
    ...
    public void dumpInternalPropertiesToLog() {
        HashMap<String, Object> internalProperties =
                (HashMap<String, Object>) getInternalProperties();
        ...
        if (internalProperties != null) {
            for (String key : internalProperties.keySet()) {
                internalPropertiesDump
                        .append("\n")
                        .append(key)
                        .append(": `")
                        .append(internalProperties.get(key))
                        .append("`");
            }
        }
