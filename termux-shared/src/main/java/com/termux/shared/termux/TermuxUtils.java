...
public class TermuxUtils {
    ...
    public enum AppInfoMode {
        ...
        /**
         * Get info for Termux app and its plugins listed in {@link
         * TermuxConstants#TERMUX_PLUGIN_APP_PACKAGE_NAMES_LIST}.
         */
    ...
    public static Context getTermuxPackageContextWithCode(@NonNull Context context) {
        return PackageUtils.getContextForPackage(
                context, TermuxConstants.TERMUX_PACKAGE_NAME, Context.CONTEXT_INCLUDE_CODE);
    }
    ...
    public static Context getTermuxStylingPackageContext(@NonNull Context context) {
        return PackageUtils.getContextForPackage(
                context, TermuxConstants.TERMUX_STYLING_PACKAGE_NAME);
    }
    ...
    public static Context getTermuxTaskerPackageContext(@NonNull Context context) {
        return PackageUtils.getContextForPackage(
                context, TermuxConstants.TERMUX_TASKER_PACKAGE_NAME);
    }
    ...
    public static Context getTermuxWidgetPackageContext(@NonNull Context context) {
        return PackageUtils.getContextForPackage(
                context, TermuxConstants.TERMUX_WIDGET_PACKAGE_NAME);
    }

    /**
     * Wrapper for {@link PackageUtils#getContextForPackageOrExitApp(Context, String, boolean,
     * String)}.
     */
    public static Context getContextForPackageOrExitApp(
            @NonNull Context context, String packageName, final boolean exitAppOnError) {
        return PackageUtils.getContextForPackageOrExitApp(
                context, packageName, exitAppOnError, TermuxConstants.TERMUX_GITHUB_REPO_URL);
    }
     ...
     *
     * <p>If your third-party app is targeting sdk `30` (android `11`), then it needs to add
     * `com.termux` package to the `queries` element or request `QUERY_ALL_PACKAGES` permission in
     * its `AndroidManifest.xml`. Otherwise it will get `PackageSetting{...... com.termux/......}
     * BLOCKED` errors in `logcat` and `RUN_COMMAND` won't work. Check
     * [package-visibility](https://developer.android.com/training/basics/intents/package-visibility#package-name),
     * `QUERY_ALL_PACKAGES` [googleplay
     * policy](https://support.google.com/googleplay/android-developer/answer/10158779 and this
     * [article](https://medium.com/androiddevelopers/working-with-package-visibility-dc252829de2d)
     * for more info.
     *
     * <p>{@code <manifest <queries> <package android:name="com.termux" /> </queries> </manifest> }
     *
     * @param context The context for operations.
     * @return Returns {@code errmsg} if {@link TermuxConstants#TERMUX_PACKAGE_NAME} is not
     *     installed or disabled, otherwise {@code null}.
     */
    public static String isTermuxAppInstalled(@NonNull final Context context) {
        return PackageUtils.isAppInstalled(
                context, TermuxConstants.TERMUX_APP_NAME, TermuxConstants.TERMUX_PACKAGE_NAME);
    }

    /**
     * Check if Termux:API app is installed and enabled. This can be used by external apps that
     * don't share `sharedUserId` with the Termux:API app.
     *
     * @param context The context for operations.
     * @return Returns {@code errmsg} if {@link TermuxConstants#TERMUX_API_PACKAGE_NAME} is not
     *     installed or disabled, otherwise {@code null}.
     */
     ...
     *
     * <p>This is done by checking if first checking if app is installed and enabled and then if
     * {@code currentPackageContext} can be used to get the {@link Context} of the app with {@link
     * TermuxConstants#TERMUX_PACKAGE_NAME} and then if {@link
     * TermuxConstants#TERMUX_PREFIX_DIR_PATH} exists and has {@link
     * FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS} permissions. The directory will not be
     * automatically created and neither the missing permissions automatically set.
     *
     * @param currentPackageContext The context of current package.
     * @return Returns {@code errmsg} if failed to get termux package {@link Context} or {@link
     *     TermuxConstants#TERMUX_PREFIX_DIR_PATH} is accessible, otherwise {@code null}.
     */
    public static String isTermuxAppAccessible(@NonNull final Context currentPackageContext) {
        ...
        if (errmsg == null) {
            Context termuxPackageContext =
                    TermuxUtils.getTermuxPackageContext(currentPackageContext);
            // If failed to get Termux app package context
            if (termuxPackageContext == null)
                errmsg =
                        currentPackageContext.getString(
                                R.string.error_termux_app_package_context_not_accessible);

            if (errmsg == null) {
                // If TermuxConstants.TERMUX_PREFIX_DIR_PATH is not a directory or does not have
     ...
     * APK installed on the device. This can only be used by apps that share `sharedUserId` with the
     * Termux app.
     *
     * <p>This is a wrapper for {@link #getTermuxAppAPKClassField(Context, String, String)}.
     *
     * @param currentPackageContext The context of current package.
     ...
     */
    public static Object getTermuxAppAPKBuildConfigClassField(
            @NonNull Context currentPackageContext, @NonNull String fieldName) {
        return getTermuxAppAPKClassField(
                currentPackageContext, TERMUX_APP.BUILD_CONFIG_CLASS_NAME, fieldName);
    }

    /**
     * Get a field value from a class of the Termux app APK installed on the device. This can only
     * be used by apps that share `sharedUserId` with the Termux app.
     *
     * <p>This is done by getting first getting termux app package context and then getting in class
     ...
     *
     * <p>Note that the value returned is from the APK file and not the current value loaded in
     * Termux app process, so only default values will be returned.
     *
     * <p>Trying to access {@code null} fields will result in {@link NoSuchFieldException}.
     *
     * @param currentPackageContext The context of current package.
     ...
     *     to get termux app package context.
     */
    public static Object getTermuxAppAPKClassField(
            @NonNull Context currentPackageContext,
            @NonNull String clazzName,
            @NonNull String fieldName) {
        try {
            Context termuxPackageContext =
                    TermuxUtils.getTermuxPackageContextWithCode(currentPackageContext);
            ...
            Logger.logStackTraceWithMessage(
                    LOG_TAG,
                    "Failed to get \"" + fieldName + "\" value from \"" + clazzName + "\" class",
                    e);
            return null;
        }
    }
    ...
    /**
     * Returns {@code true} if {@link Uri} has `package:` scheme for {@link
     * TermuxConstants#TERMUX_PACKAGE_NAME} or its sub plugin package.
     */
    public static boolean isUriDataForTermuxOrPluginPackage(@NonNull Uri data) {
        return data.toString().equals("package:" + TermuxConstants.TERMUX_PACKAGE_NAME)
                || data.toString()
                        .startsWith("package:" + TermuxConstants.TERMUX_PACKAGE_NAME + ".");
    }

    /**
     * Returns {@code true} if {@link Uri} has `package:` scheme for {@link
     * TermuxConstants#TERMUX_PACKAGE_NAME} sub plugin package.
     */
    ...
    public static void sendTermuxOpenedBroadcast(@NonNull Context context) {
        ...
        List<ResolveInfo> matches =
                context.getPackageManager().queryBroadcastReceivers(broadcast, 0);
     ...
     */
    public static String getAppInfoMarkdownString(
            final Context currentPackageContext, final AppInfoMode appInfoMode) {
     ...
     */
    public static String getAppInfoMarkdownString(
            final Context currentPackageContext,
            final AppInfoMode appInfoMode,
        ...
        switch (appInfoMode) {
            ...
            case TERMUX_AND_PLUGIN_PACKAGES:
                ...
                String termuxPluginAppsInfo =
                        TermuxUtils.getTermuxPluginAppsInfoMarkdownString(currentPackageContext);
            ...
            case TERMUX_AND_CALLING_PACKAGE:
                ...
                if (!DataUtils.isNullOrEmpty(callingPackageName)) {
                    ...
                    if (TermuxConstants.TERMUX_PLUGIN_APP_PACKAGE_NAMES_LIST.contains(
                            callingPackageName)) {
                        Context termuxPluginAppContext =
                                PackageUtils.getContextForPackage(
                                        currentPackageContext, callingPackageName);
                        if (termuxPluginAppContext != null)
                            appInfo.append(getAppInfoMarkdownString(termuxPluginAppContext, false));
                        else
                            callingPackageAppInfo =
                                    AndroidUtils.getAppInfoMarkdownString(
                                            currentPackageContext, callingPackageName);
                    } else {
                        callingPackageAppInfo =
                                AndroidUtils.getAppInfoMarkdownString(
                    ...
                    if (callingPackageAppInfo != null) {
                        ApplicationInfo applicationInfo =
                                PackageUtils.getApplicationInfoForPackage(
                                        currentPackageContext, callingPackageName);
                        if (applicationInfo != null) {
                            appInfo.append("\n\n## ")
                                    .append(
                                            PackageUtils.getAppNameForPackage(
                                                    currentPackageContext, applicationInfo))
                                    .append(" App Info\n");
     ...
     * @return Returns the markdown {@link String}.
     */
    public static String getTermuxPluginAppsInfoMarkdownString(
            final Context currentPackageContext) {
        if (currentPackageContext == null) return "null";

        StringBuilder markdownString = new StringBuilder();

        List<String> termuxPluginAppPackageNamesList =
                TermuxConstants.TERMUX_PLUGIN_APP_PACKAGE_NAMES_LIST;
        ...
        if (termuxPluginAppPackageNamesList != null) {
            for (int i = 0; i < termuxPluginAppPackageNamesList.size(); i++) {
                ...
                Context termuxPluginAppContext =
                        PackageUtils.getContextForPackage(
                                currentPackageContext, termuxPluginAppPackageName);
                // If the package context for the plugin app is not null, then assume its installed
                // and get its info
                if (termuxPluginAppContext != null) {
        ...
        if (markdownString.toString().isEmpty()) return null;
     ...
     * from the {@link TermuxConstants#TERMUX_PACKAGE_NAME} package context, then this function must
     * have been called by a different package like a plugin, so we return info for both packages if
     * {@code returnTermuxPackageInfoToo} is {@code true}.
     *
     * @param currentPackageContext The context of current package.
     * @param returnTermuxPackageInfoToo If set to {@code true}, then will return info of the {@link
     *     TermuxConstants#TERMUX_PACKAGE_NAME} package as well if its different from current
     *     package.
     * @return Returns the markdown {@link String}.
     */
    public static String getAppInfoMarkdownString(
            final Context currentPackageContext, final boolean returnTermuxPackageInfoToo) {
        if (currentPackageContext == null) return "null";
        ...
        boolean isTermuxPackage =
                (termuxPackageName != null && termuxPackageName.equals(currentPackageName));
    ...
    /**
     * Get a markdown {@link String} for the app info for the package associated with the {@code
     * context}.
     *
    ...
    public static String getAppInfoMarkdownStringInner(@NonNull final Context context) {
        ...
        if (error != null) {
            AndroidUtils.appendPropertyToMarkdown(
                    markdownString, "TERMUX_FILES_DIR", TermuxConstants.TERMUX_FILES_DIR_PATH);
            AndroidUtils.appendPropertyToMarkdown(
                    markdownString,
        ...
        if (signingCertificateSHA256Digest != null) {
            AndroidUtils.appendPropertyToMarkdown(
                    markdownString, "APK_RELEASE", getAPKRelease(signingCertificateSHA256Digest));
            AndroidUtils.appendPropertyToMarkdown(
                    markdownString,
    ...
    public static String getReportIssueMarkdownString(@NonNull final Context context) {
        ...
        markdownString
                ...
                .append(
                        context.getString(
                                R.string.msg_report_issue, TermuxConstants.TERMUX_WIKI_URL))
                .append("\n");
    ...
    public static String getImportantLinksMarkdownString(@NonNull final Context context) {
        ...
        markdownString
                ...
                .append(
                        MarkdownUtils.getLinkMarkdownString(
                                TermuxConstants.TERMUX_WIKI, TermuxConstants.TERMUX_WIKI_URL))
                .append("  ");
        markdownString
                .append("\n")
                .append(
     ...
     *
     * <p>This will take a few seconds to run due to running {@code apt update} command.
     *
     * @param context The context for operations.
     * @return Returns the markdown {@link String}.
     */
    public static String geAPTInfoMarkdownString(@NonNull final Context context) {
        ...
        InputStream inputStream =
                context.getResources().openRawResource(com.termux.shared.R.raw.apt_info_script);
        try {
        ...
        aptInfoScript =
                aptInfoScript.replaceAll(
                        Pattern.quote("@TERMUX_PREFIX@"), TermuxConstants.TERMUX_PREFIX_DIR_PATH);
        ...
        markdownString
                .append("## ")
                .append(TermuxConstants.TERMUX_APP_NAME)
                .append(" APT Info\n\n");
    ...
    public static String getTermuxDebugMarkdownString(@NonNull final Context context) {
        ...
        if (statInfo != null && logcatInfo != null) return statInfo + "\n\n" + logcatInfo;
        else if (statInfo != null) return statInfo;
        else return logcatInfo;
    }
    ...
    public static String getLogcatDumpMarkdownString(@NonNull final Context context) {
        // Build script
        // We need to prevent OutOfMemoryError since StreamGobbler StringBuilder +
