 ...
 *
 * <p>The static {@link #termuxAmSocketServer} variable stores the {@link LocalSocketManager} for
 * the {@link AmSocketServer}.
 *
 * <p>The {@link TermuxAmSocketServerClient} extends the {@link AmSocketServer.AmSocketServerClient}
 * class to also show plugin error notifications for errors and disallowed client connections in
 * addition to logging the messages to logcat, which are only logged by {@link
 * LocalSocketManagerClientBase} if log level is debug or higher for privacy issues.
 *
 * <p>It uses a filesystem socket server with the socket file at {@link
 * TermuxConstants.TERMUX_APP#TERMUX_AM_SOCKET_FILE_PATH}. It would normally only allow processes
 * belonging to the termux user and root user to connect to it. If commands are sent by the root
 * user, then the am commands executed will be run as the termux user and its permissions,
 * capabilities and selinux context instead of root.
 *
 * <p>The `$PREFIX/bin/termux-am` client connects to the server via `$PREFIX/bin/termux-am-socket`
 * to run the am commands. It provides similar functionality to "$PREFIX/bin/am" (and
 * "/system/bin/am"), but should be faster since it does not require starting a dalvik vm for every
 * command as done by "am" via termux/TermuxAm.
 *
 * <p>The server is started by termux-app Application class but is not started if {@link
 * TermuxPropertyConstants#KEY_RUN_TERMUX_AM_SOCKET_SERVER} is `false` which can be done by adding
 * the prop with value "false" to the "~/.termux/termux.properties" file. Changes require termux-app
 * to be force stopped and restarted.
 *
 * <p>The current state of the server can be checked with the {@link
 * TermuxAppShellEnvironment#ENV_TERMUX_APP__AM_SOCKET_SERVER_ENABLED} env variable, which is
 * exported for all shell sessions and tasks.
 *
 * <p>https://github.com/termux/termux-am-socket https://github.com/termux/TermuxAm
 */
public class TermuxAmSocketServer {
    ...
    @Keep protected static Boolean TERMUX_APP_AM_SOCKET_SERVER_ENABLED;

    /**
     * Setup the {@link AmSocketServer} {@link LocalServerSocket} and start listening for new {@link
     * LocalClientSocket} if enabled.
     *
    ...
    public static void setupTermuxAmSocketServer(@NonNull Context context) {
        ...
        if (TermuxAppSharedProperties.getProperties().shouldRunTermuxAmSocketServer()) {
            ...
            Logger.logDebug(
                    LOG_TAG, "Not starting " + TITLE + " socket server since its not enabled");
        }

        // Once termux-app has started, the server state must not be changed since the variable is
        // exported in shell sessions and tasks and if state is changed, then env of older shells
        // will
    ...
    /**
     * Create the {@link AmSocketServer} {@link LocalServerSocket} and start listening for new
     * {@link LocalClientSocket}.
     */
    public static synchronized void start(@NonNull Context context) {
        stop();

        AmSocketServerRunConfig amSocketServerRunConfig =
                new AmSocketServerRunConfig(
                        TITLE,
    ...
    /**
     * Stop the {@link AmSocketServer} {@link LocalServerSocket} and stop listening for new {@link
     * LocalClientSocket}.
     */
    ...
    /**
     * Show an error notification on the {@link
     * TermuxConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID} {@link
     * TermuxConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME} with a call to {@link
     * TermuxPluginUtils#sendPluginCommandErrorNotification(Context, String, CharSequence, String,
     * String)}.
     *
     ...
     */
    public static synchronized void showErrorNotification(
        ...
        TermuxPluginUtils.sendPluginCommandErrorNotification(
                ...
                LocalSocketManager.getErrorMarkdownString(
                        error, localSocketRunConfig, clientSocket));
    }

    public static Boolean getTermuxAppAMSocketServerEnabled(
            @NonNull Context currentPackageContext) {
        boolean isTermuxApp =
                TermuxConstants.TERMUX_PACKAGE_NAME.equals(currentPackageContext.getPackageName());
        if (isTermuxApp) {
            return TERMUX_APP_AM_SOCKET_SERVER_ENABLED;
        } else {
            // Currently, unsupported since plugin app processes don't know that value is set in
            // termux
            // app process TermuxAmSocketServer class. A binder API or a way to check if server is
            // actually
            // running needs to be used. Long checks would also not be possible on main application
            // thread
            return null;
        }
    }
    ...
    /**
     * Enhanced implementation for {@link AmSocketServer.AmSocketServerClient} for {@link
     * TermuxAmSocketServer}.
     */
    public static class TermuxAmSocketServerClient extends AmSocketServer.AmSocketServerClient {
        ...
        public Thread.UncaughtExceptionHandler getLocalSocketManagerClientThreadUEH(
            ...
            // Use termux crash handler for socket listener thread just like used for main app
            // process thread.
        ...
        public void onError(
            ...
            if (localSocketManager.isRunning()) {
                TermuxAmSocketServer.showErrorNotification(
