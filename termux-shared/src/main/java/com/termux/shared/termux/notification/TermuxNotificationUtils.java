...
public class TermuxNotificationUtils {
     ...
     *
     * <p>Termux app and its plugin must use unique notification ids from the same pool due to usage
     * of android:sharedUserId.
    ...
    public static Notification.Builder getTermuxOrPluginAppNotificationBuilder(
        ...
        // resource id for the plugin app with setSmallIcon(@DrawableRes int icon) since
        // notification
        // is shown with termuxPackageContext and termux-app package would have a different id and
        // when android tries to load the drawable an exception would be thrown and notification
        // will
        // not be thrown.
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            builder.setSmallIcon(
                    Icon.createWithResource(
                            currentPackageContext, R.drawable.ic_error_notification));
        } else {
            // Set drawable resource id used by termux-app package
            Integer iconResId =
                    ResourceUtils.getDrawableResourceId(
                            termuxPackageContext,
                            "ic_error_notification",
                            termuxPackageContext.getPackageName(),
                            true);
            if (iconResId != null) builder.setSmallIcon(iconResId);
        }
