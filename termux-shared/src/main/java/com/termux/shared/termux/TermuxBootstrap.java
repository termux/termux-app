...
public class TermuxBootstrap {
    ...
    /**
     * The field name used by Termux app to store package variant in {@link
    ...
    /**
     * Set {@link #TERMUX_APP_PACKAGE_VARIANT} and {@link #TERMUX_APP_PACKAGE_MANAGER} from {@code
     * packageVariantName} passed.
     */
    public static void setTermuxPackageManagerAndVariant(@Nullable String packageVariantName) {
        ...
        if (TERMUX_APP_PACKAGE_VARIANT == null) {
            throw new RuntimeException(
                    "Unsupported TERMUX_APP_PACKAGE_VARIANT \"" + packageVariantName + "\"");
        }

        Logger.logVerbose(
                LOG_TAG,
                "Set TERMUX_APP_PACKAGE_VARIANT to \"" + TERMUX_APP_PACKAGE_VARIANT + "\"");
        ...
        if (TERMUX_APP_PACKAGE_MANAGER == null) {
            throw new RuntimeException(
                    "Unsupported TERMUX_APP_PACKAGE_MANAGER \""
                            + packageManagerName
                            + "\" with variant \""
                            + packageVariantName
                            + "\"");
        ...
        Logger.logVerbose(
                LOG_TAG,
                "Set TERMUX_APP_PACKAGE_MANAGER to \"" + TERMUX_APP_PACKAGE_MANAGER + "\"");
    }

    /**
     * Set {@link #TERMUX_APP_PACKAGE_VARIANT} and {@link #TERMUX_APP_PACKAGE_MANAGER} with the
     * {@link #BUILD_CONFIG_FIELD_TERMUX_PACKAGE_VARIANT} field value from the {@link
     * TERMUX_APP#BUILD_CONFIG_CLASS_NAME} class of the Termux app APK installed on the device. This
     * can only be used by apps that share `sharedUserId` with the Termux app and can be used by
     * plugin apps.
     *
     * @param currentPackageContext The context of current package.
     */
    public static void setTermuxPackageManagerAndVariantFromTermuxApp(
            @NonNull Context currentPackageContext) {
        String packageVariantName =
                getTermuxAppBuildConfigPackageVariantFromTermuxApp(currentPackageContext);
        if (packageVariantName != null) {
            TermuxBootstrap.setTermuxPackageManagerAndVariant(packageVariantName);
        } else {
            Logger.logError(
                    LOG_TAG,
                    "Failed to set TERMUX_APP_PACKAGE_VARIANT and TERMUX_APP_PACKAGE_MANAGER from"
                        + " the termux app");
        }
    }

    /**
     * Get {@link #BUILD_CONFIG_FIELD_TERMUX_PACKAGE_VARIANT} field value from the {@link
     * TERMUX_APP#BUILD_CONFIG_CLASS_NAME} class of the Termux app APK installed on the device. This
     * can only be used by apps that share `sharedUserId` with the Termux app.
     *
     * @param currentPackageContext The context of current package.
     * @return Returns the field value, otherwise {@code null} if an exception was raised or failed
     *     to get termux app package context.
     */
    public static String getTermuxAppBuildConfigPackageVariantFromTermuxApp(
            @NonNull Context currentPackageContext) {
        try {
            return (String)
                    TermuxUtils.getTermuxAppAPKBuildConfigClassField(
                            currentPackageContext, BUILD_CONFIG_FIELD_TERMUX_PACKAGE_VARIANT);
        } catch (Exception e) {
            Logger.logStackTraceWithMessage(
                    ...
                    "Failed to get \""
                            ...
                            + "\" value from \""
                            + TERMUX_APP.BUILD_CONFIG_CLASS_NAME
                            + "\" class",
                    e);
            return null;
        }
    ...
    public enum PackageManager {
         ...
         * https://wiki.debian.org/Apt https://wiki.debian.org/deb
         */
        APT("apt");
    ...
    /**
     * Termux package variant. The substring before first dash "-" must match one of the {@link
     * PackageManager}.
     */
