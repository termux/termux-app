 ...
 * {@link ReportInfo}. Add Following to `AndroidManifest.xml` to use in an app: {@code `<activity
 * android:name="com.termux.shared.activities.ReportActivity"
 * android:theme="@style/Theme.AppCompat.TermuxReportActivity"
 * android:documentLaunchMode="intoExisting" />` } and {@code `<receiver
 ...
 *
 * <p>Also make an incremental call to {@link #deleteReportInfoFilesOlderThanXDays(Context, int,
 * boolean)} in the app to cleanup cached files.
 */
public class ReportActivity extends AppCompatActivity {
    ...
    private void updateUI() {
        ...
        if (mBundle.containsKey(EXTRA_REPORT_INFO_OBJECT_FILE_PATH)) {
            ...
            Logger.logVerbose(
                    LOG_TAG,
                    ReportInfo.class.getSimpleName()
                            + " serialized object will be read from file at path \""
                            + mReportInfoFilePath
                            + "\"");
            if (mReportInfoFilePath != null) {
                try {
                    FileUtils.ReadSerializableObjectResult result =
                            FileUtils.readSerializableObjectFromFile(
                                    ReportInfo.class.getSimpleName(),
                                    mReportInfoFilePath,
                                    ReportInfo.class,
                                    false);
                    ...
                    Logger.logStackTraceWithMessage(
                            LOG_TAG,
                            "Failure while getting "
                                    + ReportInfo.class.getSimpleName()
                                    + " serialized object from file at path \""
                                    + mReportInfoFilePath
                                    + "\"",
                            e);
                }
        ...
        final MarkwonAdapter adapter =
                MarkwonAdapter.builderTextViewIsRoot(R.layout.markdown_adapter_node_default)
                        .include(
                                FencedCodeBlock.class,
                                SimpleEntry.create(
                                        R.layout.markdown_adapter_node_code_block,
                                        R.id.code_text_view))
                        .build();
    ...
    public boolean onOptionsItemSelected(final MenuItem item) {
        ...
        if (id == R.id.menu_item_share_report) {
            ...
            ShareUtils.copyTextToClipboard(
                    this, ReportInfo.getReportInfoMarkdownString(mReportInfo), null);
        } else if (id == R.id.menu_item_save_report_to_file) {
            ShareUtils.saveTextToFile(
                    this,
                    mReportInfo.reportSaveFileLabel,
                    mReportInfo.reportSaveFilePath,
                    ReportInfo.getReportInfoMarkdownString(mReportInfo),
                    true,
    ...
    private void generateReportActivityMarkdownString() {
        ...
        if (reportMarkdownStringSize > ACTIVITY_TEXT_SIZE_LIMIT_IN_BYTES) {
            Logger.logVerbose(
                    LOG_TAG,
                    mReportInfo.reportTitle
                            + " report string size "
                            + reportMarkdownStringSize
                            + " is greater than "
        ...
        if (reportStringSize > ACTIVITY_TEXT_SIZE_LIMIT_IN_BYTES) {
            ...
            Logger.logVerbose(
                    LOG_TAG,
                    mReportInfo.reportTitle
                            + " report string total size "
                            + reportStringSize
                            + " is greater than "
                            + ACTIVITY_TEXT_SIZE_LIMIT_IN_BYTES
                            + " and will be truncated");
            mReportActivityMarkdownString =
                    this.getString(R.string.msg_report_truncated)
                            + DataUtils.getTruncatedCommandOutput(
    ...
    public static class NewInstanceResult {
         ...
         * by a call to {@link android.app.PendingIntent#getBroadcast(Context, int, Intent, int)} so
         * that {@link ReportActivityBroadcastReceiver} can do cleanup of {@link
         * #EXTRA_REPORT_INFO_OBJECT_FILE_PATH}.
         */
    ...
    public static void startReportActivity(
            @NonNull final Context context, @NonNull ReportInfo reportInfo) {
     ...
     * and do cleanup.
     *
     * <p>If {@link ReportInfo} size is too large, then a TransactionTooLargeException will be
     * thrown so its object may be saved to a file in the {@link Context#getCacheDir()}. Then when
     * activity starts, its read back and the file is deleted in {@link #onDestroy()}. Note that
     * files may still be left if {@link #onDestroy()} is not called or doesn't finish. A separate
     * cleanup routine is implemented from that case by {@link
     * #deleteReportInfoFilesOlderThanXDays(Context, int, boolean)} which should be called
     ...
     */
    @NonNull
    public static NewInstanceResult newInstance(
            @NonNull final Context context, @NonNull final ReportInfo reportInfo) {
        ...
        if (size > DataUtils.TRANSACTION_SIZE_LIMIT_IN_BYTES) {
            ...
            Logger.logVerbose(
                    ...
                    reportInfo.reportTitle
                            ...
                            + " and it will be written to file at path \""
                            + reportInfoFilePath
                            + "\"");
            Error error =
                    FileUtils.writeSerializableObjectToFile(
                            ReportInfo.class.getSimpleName(), reportInfoFilePath, reportInfo);
            if (error != null) {
    ...
    private static Intent createContentIntent(
        ...
        // Note that ReportActivity should have `documentLaunchMode="intoExisting"` set in
        // `AndroidManifest.xml`
    ...
    private static Intent createDeleteIntent(
            @NonNull final Context context, final String reportInfoFilePath) {
    ...
    private static String getReportInfoDirectoryPath(Context context) {
        // Canonicalize to solve /data/data and /data/user/0 issues when comparing with
        // reportInfoFilePath
    ...
    private static void deleteReportInfoFile(Context context, String reportInfoFilePath) {
        ...
        // Extra protection for mainly if someone set `exported="true"` for
        ...
        if (!reportInfoFilePath.equals(reportInfoDirectoryPath)
            ...
            Logger.logVerbose(
                    ...
                    "Deleting "
                            + ReportInfo.class.getSimpleName()
                            + " serialized object file at path \""
                            + reportInfoFilePath
                            + "\"");
            Error error =
                    FileUtils.deleteRegularFile(
                            ReportInfo.class.getSimpleName(), reportInfoFilePath, true);
            if (error != null) {
                Logger.logErrorExtended(LOG_TAG, error.toString());
            }
        } else {
     ...
     * ReportActivity, then opening the notification will throw a file not found error, so choose
     * days value appropriately or check if a notification is still active if tracking notification
     * ids. The {@link Context} object passed must be of the same package with which {@link
     * #newInstance(Context, ReportInfo)} was called since a call to {@link Context#getCacheDir()}
     * is made.
     *
     * @param context The {@link Context} for operations.
     ...
     *     thread and results returned synchronously. If set to {@code false}, then a new thread is
     *     started run the commands asynchronously in the background and control is returned to the
     *     caller thread.
     * @return Returns the {@code error} if deleting was not successful, otherwise {@code null}.
     */
    public static Error deleteReportInfoFilesOlderThanXDays(
    ...
    private static Error deleteReportInfoFilesOlderThanXDaysInner(
            @NonNull final Context context, int days) {
        ...
        Logger.logVerbose(
                ...
                "Deleting "
                        + ReportInfo.class.getSimpleName()
                        + " serialized object files under directory path \""
                        + reportInfoDirectoryPath
                        + "\" older than "
                        + days
                        + " days");
            ...
            if (ACTION_DELETE_REPORT_INFO_OBJECT_FILE.equals(action)) {
                ...
                if (bundle.containsKey(EXTRA_REPORT_INFO_OBJECT_FILE_PATH)) {
                    deleteReportInfoFile(
                            context, bundle.getString(EXTRA_REPORT_INFO_OBJECT_FILE_PATH));
                }
            }
        }
    }
