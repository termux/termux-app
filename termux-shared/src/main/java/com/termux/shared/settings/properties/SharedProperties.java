 ...
 * the key/value pairs when an instance object is used. Operations are done under synchronization
 * locks and should be thread safe.
 *
 * <p>If {@link SharedProperties} instance object is used, then two types of in-memory cache maps
 * are maintained, one for the literal {@link String} values found in the file for the keys and an
 * additional one that stores (near) primitive {@link Object} values for internal use by the caller.
 *
 * <p>The {@link SharedProperties} also provides static functions that can be used to read
 * properties from files or individual key values or even their internal values. An automatic
 * mapping to a boolean as internal value can also be done. An in-memory cache is not maintained,
 * nor are locks used.
 *
 * <p>This currently only has read support, write support can/will be added later if needed. Check
 * android's SharedPreferencesImpl class for reference implementation.
 *
 * <p>https://cs.android.com/android/platform/superproject/+/android-11.0.0_r3:frameworks/base/core/java/android/app/SharedPreferencesImpl.java
 */
public class SharedProperties {
    ...
    /**
     * The {@link HashMap<>} object that maintains an in-memory cache of internal values for the
     * values loaded from the {@link #mPropertiesFile} file. The key/value pairs are of any keys
     * defined by {@link #mPropertiesList} that are found in the file against their internal {@link
     * Object} values returned by the call to {@link
     * SharedPropertiesParser#getInternalPropertyValueFromValue(Context, String, String)} interface.
     */
     ...
     *     If this is set to {@code null}, then all properties that exist in {@code propertiesFile}
     *     will be read by {@link #loadPropertiesFromDisk()}
     * @param sharedPropertiesParser The implementation of the {@link SharedPropertiesParser}
     *     interface.
     */
    public SharedProperties(
            @NonNull Context context,
            @Nullable File propertiesFile,
     ...
     * in-memory cache. Properties are not loading automatically when constructor is called and must
     * be manually called.
     */
            ...
            for (String key : propertiesList) {
                value =
                        properties.getProperty(
                                key); // value will be null if key does not exist in propertiesFile
                ...
                internalValue =
                        mSharedPropertiesParser.getInternalPropertyValueFromValue(
                                mContext, key, value);

                // If the internal value was successfully added to map, then also add value to
                // newProperties
                ...
                if (putToMap(map, key, internalValue)) { // null internalValue will be put into map
                    putToProperties(
                            newProperties,
                            key,
                            value); // null value will **not** be into properties
                }
            }
     ...
     * value is {@code false}.
     *
     * @param cached If {@code true}, then the {@link #mProperties} in-memory cache is returned.
     *     Otherwise the {@link Properties} object is directly read from the {@link
     *     #mPropertiesFile}.
     * @return Returns the {@link Properties} object if read from file, otherwise a copy of {@link
     *     #mProperties}.
     */
     ...
     * @param cached If {@code true}, then the value is returned from the {@link #mProperties}
     *     in-memory cache. Otherwise the {@link Properties} object is read directly from the {@link
     *     #mPropertiesFile} and value is returned from it against the key.
    ...
    /**
     * Get the {@link #mMap} object for the {@link #mPropertiesFile}. A call to {@link
     ...
     * The value is returned from the {@link #mMap} in-memory cache, so a call to {@link
     * #loadPropertiesFromDisk()} must be made before this.
     *
    ...
    public static Properties getPropertiesFromFile(
        ...
        try {
            ...
            Logger.logStackTraceWithMessage(
                    LOG_TAG,
                    "Error loading properties file \"" + propertiesFile.getAbsolutePath() + "\"",
                    e);
            return null;
        }
    ...
    /**
     * Returns the first {@link File} found in {@code propertiesFilePaths} from which app properties
     * can be loaded. If the {@link File} found is not a regular file or is not readable, then
     * {@code null} is returned. Symlinks **will not** be followed for potential security reasons.
     *
     ...
     */
    public static File getPropertiesFileFromList(
            List<String> propertiesFilePaths, @NonNull String logTag) {
        if (propertiesFilePaths == null || propertiesFilePaths.size() == 0) return null;
     ...
     *
     * @param context The {@link Context} for the {@link
     *     #getPropertiesFromFile(Context,File,SharedPropertiesParser)} call.
     * @param propertiesFile The {@link File} to read the {@link Properties} from.
                ...
                getDefaultIfNull(
                        getDefaultIfNull(
                                        getPropertiesFromFile(
                                                context, propertiesFile, sharedPropertiesParser),
                                        new Properties())
                                .get(key),
                        def);
    }
     ...
     * @param sharedPropertiesParser The implementation of the {@link SharedPropertiesParser}
     *     interface.
     * @return Returns the {@link String} Object returned by the call to {@link
     *     SharedPropertiesParser#getInternalPropertyValueFromValue(Context,String,String)}.
     */
    public static Object getInternalProperty(
            Context context,
            File propertiesFile,
            String key,
            @NonNull SharedPropertiesParser sharedPropertiesParser) {
    ...
    public static boolean isPropertyValueTrue(
            Context context, File propertiesFile, String key, boolean logErrorOnInvalidValue) {
    ...
    public static boolean isPropertyValueTrue(
            ...
            @Nullable SharedPropertiesParser sharedPropertiesParser) {
        return (boolean)
                getBooleanValueForStringValue(
                        key,
                        (String)
                                getProperty(
                                        context, propertiesFile, key, null, sharedPropertiesParser),
                        false,
                        logErrorOnInvalidValue,
                        LOG_TAG);
    }

    public static boolean isPropertyValueFalse(
            Context context, File propertiesFile, String key, boolean logErrorOnInvalidValue) {
    ...
    public static boolean isPropertyValueFalse(
            ...
            @Nullable SharedPropertiesParser sharedPropertiesParser) {
        return (boolean)
                getInvertedBooleanValueForStringValue(
                        key,
                        (String)
                                getProperty(
                                        context, propertiesFile, key, null, sharedPropertiesParser),
    ...
    /**
     * Put a value in a {@link #mMap}. The key cannot be {@code null}. Only {@code null}, primitive
     * or their wrapper classes or String class objects are allowed to be added to the map, although
     * this limitation may be changed.
     *
    ...
    public static boolean putToMap(HashMap<String, Object> map, String key, Object value) {
        ...
        if (put) {
            ...
            Logger.logError(
                    LOG_TAG,
                    "Cannot put a non-primitive value for the key \""
                            + key
                            + "\" into properties map");
            return false;
        }
    ...
    /**
     * Put a value in a {@link Map}. The key cannot be {@code null}. Passing {@code null} as the
     * value argument is equivalent to removing the key from the properties.
     *
    ...
    public static boolean putToProperties(Properties properties, String key, String value) {
        ...
        if (properties == null) {
            Logger.logError(
                    LOG_TAG, "Properties passed to SharedProperties.putToProperties() is null");
            return false;
        }
     ...
     * @return Returns {@code true} or {@code false} if value is the literal string "true" or
     *     "false" respectively, regardless of case. Otherwise returns {@code null}.
     */
     ...
     * @param logTag If log tag to use for logging errors.
     * @return Returns {@code true} or {@code false} if value is the literal string "true" or
     *     "false" respectively, regardless of case. Otherwise returns default value.
     */
    public static boolean getBooleanValueForStringValue(
     ...
     *     was not {@code null} and was invalid.
     * @param logTag If log tag to use for logging errors.
     * @return Returns {@code true} or {@code false} if value is the literal string "false" or
     *     "true" respectively, regardless of case. Otherwise returns default value.
     */
    public static boolean getInvertedBooleanValueForStringValue(
            String key, String value, boolean def, boolean logErrorOnInvalidValue, String logTag) {
        return (boolean)
                getDefaultIfNotInMap(
                        key,
     ...
     *
     * @param key The shared properties {@link String} key value for which the value is being
     *     returned.
     * @param map The {@link BiMap<>} value to get the value from.
     * @param inputValue The {@link Object} key value of the map.
     * @param defaultOutputValue The default {@link boolean} value to return if {@code inputValue}
     *     not found in map. The default value must exist as a value in the {@link BiMap<>} passed.
     * @param logErrorOnInvalidValue If {@code true}, then an error will be logged if {@code
     *     inputValue} was not {@code null} and was not found in the map.
     * @param logTag If log tag to use for logging errors.
     * @return Returns the value for the {@code inputValue} key from the map if it exists. Otherwise
     *     returns default value.
     */
    public static Object getDefaultIfNotInMap(
            String key,
            @NonNull BiMap<?, ?> map,
            Object inputValue,
            ...
            if (defaultInputValue == null)
                Logger.logError(
                        ...
                        "The default output value \""
                                ...
                                + "\" for the key \""
                                + key
                                + "\" does not exist as a value in the BiMap passed to"
                                + " getDefaultIfNotInMap(): "
                                + map.values());
    ...
    /**
     * Get the {@code int} {@code value} as is if between {@code min} and {@code max} (inclusive),
     ...
     */
    public static int getDefaultIfNotInRange(
            String key,
            int value,
            int def,
            int min,
            int max,
            boolean logErrorOnInvalidValue,
            boolean ignoreErrorIfValueZero,
            String logTag) {
    ...
    /**
     * Get the {@code float} {@code value} as is if between {@code min} and {@code max} (inclusive),
     * otherwise return default value.
     *
     * @param key The shared properties {@link String} key value for which the value is being
     *     returned.
     ...
     * @return Returns the {@code value} as is if within range. Otherwise returns default value.
     */
    public static float getDefaultIfNotInRange(
            String key,
            float value,
            float def,
            float min,
