...
public class LocalSocketRunConfig implements Serializable {
     ...
     *
     * <p>For a filesystem socket, this must be an absolute path to the socket file. Creation of a
     * new socket will fail if the server starter app process does not have write and search
     * (execute) permission on the directory in which the socket is created. The client process must
     * have write permission on the socket to connect to it. Other app will not be able to connect
     * to socket if its created in private app data directory.
     *
     * <p>For an abstract namespace socket, the first byte must be a null `\0` character. Note that
     * on Android 9+, if server app is using `targetSdkVersion` `28`, then other apps will not be
     * able to connect to it due to selinux restrictions. > Per-app SELinux domains > Apps that
     * target Android 9 or higher cannot share data with other apps using world-accessible Unix
     * permissions. This change improves the integrity of the Android Application Sandbox,
     ...
     *
     * <p>Max allowed length is 108 bytes as per sun_path size (UNIX_PATH_MAX) on Linux.
     */
    ...
    /**
     * The {@link LocalServerSocket} file descriptor. Value will be `>= 0` if socket has been
     * created successfully and `-1` if not created or closed.
     */
    protected int mFD = -1;
     ...
     *
     * <p>https://manpages.debian.org/testing/manpages/socket.7.en.html
     ...
     * deadline. Defaults to {@link #DEFAULT_DEADLINE}.
     */
    ...
    /**
     * The {@link LocalServerSocket} backlog for the maximum length to which the queue of pending
     * connections for the socket may grow. This value may be ignored or may not have one-to-one
     * mapping in kernel implementation. Value must be greater than 0.
     *
     * <p>https://cs.android.com/android/platform/superproject/+/android-12.0.0_r32:frameworks/base/core/java/android/net/LocalSocketManager.java;l=31
    ...
    public LocalSocketRunConfig(
        ...
        if (mAbstractNamespaceSocket) mPath = path;
        else mPath = FileUtils.getCanonicalPath(path, null);
    }
        ...
        logString
                ...
                .append(
                        Logger.getSingleLineLogStringEntry(
                                "AbstractNamespaceSocket", mAbstractNamespaceSocket, "-"));
        logString
                .append("\n")
                .append(
                        Logger.getSingleLineLogStringEntry(
                                "LocalSocketManagerClient",
