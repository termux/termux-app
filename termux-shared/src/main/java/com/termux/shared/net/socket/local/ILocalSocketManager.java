...
public interface ILocalSocketManager {
    ...
    /**
     * This is called if any error is raised by {@link LocalSocketManager}, {@link
     * LocalServerSocket} or {@link LocalClientSocket}. The server will automatically close the
     * client socket with a call to {@link LocalClientSocket#closeClientSocket(boolean)} if the
     * error occurred due to the client.
     *
     * <p>The {@link LocalClientSocket#getPeerCred()} can be used to get the {@link PeerCred} object
     ...
     * the server app's user id or root user id. The server will automatically close the client
     * socket with a call to {@link LocalClientSocket#closeClientSocket(boolean)}.
     *
     * <p>The {@link LocalClientSocket#getPeerCred()} can be used to get the {@link PeerCred} object
     * containing info for the connected client/peer.
     *
     ...
     * @param error The {@link Error} auto generated that can be used for logging purposes.
     */
    void onDisallowedClientConnected(
            @NonNull LocalSocketManager localSocketManager,
            @NonNull LocalClientSocket clientSocket,
            @NonNull Error error);

    /**
     * This is called if a {@link LocalServerSocket} connects to the server which has the the server
     * app's user id or root user id. It is the responsibility of the interface implementation to
     * close the client socket with a call to {@link LocalClientSocket#closeClientSocket(boolean)}
