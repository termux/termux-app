 ...
 *
 * <p>Usage: 1. Implement the {@link ILocalSocketManager} that will receive call backs from the
 * server including when client connects via {@link
 * ILocalSocketManager#onClientAccepted(LocalSocketManager, LocalClientSocket)}. Optionally extend
 * the {@link LocalSocketManagerClientBase} class that provides base implementation. 2. Create a
 * {@link LocalSocketRunConfig} instance with the run config of the server. 3. Create a {@link
 * LocalSocketManager} instance and call {@link #start()}. 4. Stop server if needed with a call to
 * {@link #stop()}.
 */
public class LocalSocketManager {
    ...
    /**
     * The {@link Thread.UncaughtExceptionHandler} used for client thread started by {@link
     * LocalSocketManager}.
     */
    @NonNull protected final Thread.UncaughtExceptionHandler mLocalSocketManagerClientThreadUEH;

    /**
     * Whether the {@link LocalServerSocket} managed by {@link LocalSocketManager} in running or
     * not.
     */
     ...
     */
    public LocalSocketManager(
            @NonNull Context context, @NonNull LocalSocketRunConfig localSocketRunConfig) {
    ...
    public synchronized Error start() {
        ...
        if (!localSocketLibraryLoaded) {
            try {
                ...
                Error error =
                        LocalSocketErrno.ERRNO_START_LOCAL_SOCKET_LIB_LOAD_FAILED_WITH_EXCEPTION
                                .getError(t, LOCAL_SOCKET_LIBRARY, t.getMessage());
    ...
    */
    ...
    /**
     * Creates an AF_UNIX/SOCK_STREAM local server socket at {@code path}, with the specified
     * backlog.
     *
     * @param serverTitle The server title used for logging and errors.
     * @param path The path at which to create the socket. For a filesystem socket, this must be an
     *     absolute path to the socket file. For an abstract namespace socket, the first byte must
     *     be a null `\0` character. Max allowed length is 108 bytes as per sun_path size
     *     (UNIX_PATH_MAX) on Linux.
     * @param backlog The maximum length to which the queue of pending connections for the socket
     *     may grow. This value may be ignored or may not have one-to-one mapping in kernel
     *     implementation. Value must be greater than 0.
     * @return Returns the {@link JniResult}. If server creation was successful, then {@link
     *     JniResult#retval} will be 0 and {@link JniResult#intData} will contain the server socket
     *     fd.
     */
    @Nullable
    public static JniResult createServerSocket(
            @NonNull String serverTitle, @NonNull byte[] path, int backlog) {
        try {
     ...
     * @return Returns the {@link JniResult}. If closing socket was successful, then {@link
     ...
     * @return Returns the {@link JniResult}. If accepting socket was successful, then {@link
     *     JniResult#retval} will be 0 and {@link JniResult#intData} will contain the client socket
     *     fd.
     */
    @Nullable
     ...
     * On success, the number of bytes read is returned (zero indicates end of file). It is not an
     * error if bytes read is smaller than the number of bytes requested; this may happen for
     * example because fewer bytes are actually available right now (maybe because we were close to
     * end-of-file, or because we are reading from a pipe), or because read() was interrupted by a
     * signal. On error, the {@link JniResult#errno} and {@link JniResult#errmsg} will be set.
     *
     * <p>If while reading the deadline elapses but all the data has not been read, the call will
     * fail.
     *
     * @param serverTitle The server title used for logging and errors.
     * @param fd The socket fd.
     ...
     * @return Returns the {@link JniResult}. If reading was successful, then {@link
     *     JniResult#retval} will be 0 and {@link JniResult#intData} will contain the bytes read.
     */
    @Nullable
    public static JniResult read(
            @NonNull String serverTitle, int fd, @NonNull byte[] data, long deadline) {
        try {
    ...
    /**
     * Attempts to send data buffer to the file descriptor. On error, the {@link JniResult#errno}
     * and {@link JniResult#errmsg} will be set.
     *
     * <p>If while sending the deadline elapses but all the data has not been sent, the call will
     * fail.
     *
     * @param serverTitle The server title used for logging and errors.
     * @param fd The socket fd.
     ...
     * @param deadline The deadline milliseconds since epoch.
     * @return Returns the {@link JniResult}. If sending was successful, then {@link
     *     JniResult#retval} will be 0.
     */
    @Nullable
    public static JniResult send(
            @NonNull String serverTitle, int fd, @NonNull byte[] data, long deadline) {
        try {
     ...
     * @param serverTitle The server title used for logging and errors.
     * @param fd The socket fd.
     * @return Returns the {@link JniResult}. If checking availability was successful, then {@link
     *     JniResult#retval} will be 0 and {@link JniResult#intData} will contain the bytes
     *     available.
     */
    @Nullable
     ...
     * @return Returns the {@link JniResult}. If setting timeout was successful, then {@link
     *     JniResult#retval} will be 0.
     */
    @Nullable
    ...
    /**
     * Wrapper for {@link #onError(LocalClientSocket, Error)} for {@code null} {@link
     * LocalClientSocket}.
     */
    ...
    /**
     * Wrapper to call {@link ILocalSocketManager#onError(LocalSocketManager, LocalClientSocket,
     * Error)} in a new thread.
     */
    ...
    /**
     * Wrapper to call {@link ILocalSocketManager#onDisallowedClientConnected(LocalSocketManager,
     * LocalClientSocket, Error)} in a new thread.
     */
    public void onDisallowedClientConnected(
            @NonNull LocalClientSocket clientSocket, @NonNull Error error) {
        startLocalSocketManagerClientThread(
                () ->
                        mLocalSocketManagerClient.onDisallowedClientConnected(
                                this, clientSocket, error));
    }

    /**
     * Wrapper to call {@link ILocalSocketManager#onClientAccepted(LocalSocketManager,
     * LocalClientSocket)} in a new thread.
     */
    ...
    /**
     * All client accept logic must be run on separate threads so that incoming client acceptance is
     * not blocked.
     */
    public void startLocalSocketManagerClientThread(@NonNull Runnable runnable) {
        ...
        try {
            ...
            Logger.logStackTraceWithMessage(
                    LOG_TAG, "LocalSocketManagerClientThread start failed", e);
        }
    }
    ...
    /**
     * Get {@link Thread.UncaughtExceptionHandler} returned by call to {@link
     * ILocalSocketManager#getLocalSocketManagerClientThreadUEH(LocalSocketManager)} or the default
     * handler that just logs the exception.
     */
    ...
    public static String getErrorMarkdownString(
            @NonNull Error error,
            @NonNull LocalSocketRunConfig localSocketRunConfig,
            @Nullable LocalClientSocket clientSocket) {
    ...
    @Nullable
    private static native JniResult createServerSocketNative(
            @NonNull String serverTitle, @NonNull byte[] path, int backlog);
    ...
    @Nullable
    private static native JniResult readNative(
            @NonNull String serverTitle, int fd, @NonNull byte[] data, long deadline);

    @Nullable
    private static native JniResult sendNative(
            @NonNull String serverTitle, int fd, @NonNull byte[] data, long deadline);

    @Nullable
    private static native JniResult availableNative(@NonNull String serverTitle, int fd);

    private static native JniResult setSocketReadTimeoutNative(
            @NonNull String serverTitle, int fd, int timeout);

    @Nullable
    private static native JniResult setSocketSendTimeoutNative(
            @NonNull String serverTitle, int fd, int timeout);

    @Nullable
    private static native JniResult getPeerCredNative(
            @NonNull String serverTitle, int fd, PeerCred peerCred);
}
