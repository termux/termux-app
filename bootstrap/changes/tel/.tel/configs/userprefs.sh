...
export WEATHER_CHECK=60 # check weather forcast every x mins, used for weather status script [minutes]
...
export REBUILD_CACHES_STARTUP=true #keeps contacts and app lists up to date, disable for faster startup. but user will have to update manually. (using app -u, phone -u) [true/false]
