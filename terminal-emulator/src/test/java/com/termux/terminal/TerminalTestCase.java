...
public abstract class TerminalTestCase extends TestCase {
    ...
    public static final class ChangedTitle {
        ...
        public boolean equals(Object o) {
            ...
            return Objects.equals(oldTitle, other.oldTitle)
                    && Objects.equals(newTitle, other.newTitle);
        }
    ...
    protected TerminalTestCase withTerminalSized(int columns, int rows) {
        // The tests aren't currently using the client, so a null client will suffice, a dummy
        // client should be implemented if needed
    ...
    protected TerminalTestCase assertInvariants() {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            assertTrue(
                    "Line exists at multiple places: " + i,
                    linesSet.add(new LineWrapper(lines[i])));
            ...
            for (int j = 0; j < usedChars; j++) {
                ...
                assertFalse(
                        "Screen should never contain unassigned characters",
                        Character.getType(codePoint) == Character.UNASSIGNED);
                int width = WcWidth.width(codePoint);
                assertFalse(
                        "The first column should not start with combining character",
                        currentColumn == 0 && width < 0);
                if (width > 0) currentColumn += width;
            }
            assertEquals(
                    "Line whose width does not match screens. line="
                            + new String(lines[i].mText, 0, lines[i].getSpaceUsed()),
                    screen.mColumns,
                    currentColumn);
        }

        assertEquals(
                "The alt buffer should have have no history",
                mTerminal.mAltBuffer.mTotalRows,
                mTerminal.mAltBuffer.mScreenRows);
        if (mTerminal.isAlternateBufferActive()) {
            assertEquals(
                    "The alt buffer should be the same size as the screen",
                    mTerminal.mRows,
                    mTerminal.mAltBuffer.mTotalRows);
        }
        ...
        TerminalRow l =
                mTerminal
                        ...
                        .allocateFullLineIfNecessary(
                                mTerminal.getScreen().externalToInternalRow(line));
        char[] chars = l.mText;
    ...
    public TerminalTestCase assertLineWraps(boolean... lines) {
        for (int i = 0; i < lines.length; i++)
            assertEquals(
                    ...
                    lines[i],
                    mTerminal.getScreen()
                            .mLines[mTerminal.getScreen().externalToInternalRow(i)]
                            .mLineWrap);
        return this;
    }
    ...
    protected TerminalTestCase assertLineStartsWith(int line, int... codePoints) {
        char[] chars =
                mTerminal.getScreen()
                        .mLines[mTerminal.getScreen().externalToInternalRow(line)]
                        .mText;
        int charIndex = 0;
                ...
                if ((effectsAtCell & attributes) != attributes)
                    fail(
                            "Line="
                                    ...
                                    + j
                                    + ", expected "
                                    + describeStyle(attributes)
                                    + " set, was "
                                    + describeStyle(effectsAtCell));
            }
        }
        return this;
    }
    ...
    private static String describeStyle(int styleBits) {
        return "'"
                + ((styleBits & TextStyle.CHARACTER_ATTRIBUTE_BLINK) != 0 ? ":BLINK:" : "")
                ...
                + ((styleBits & TextStyle.CHARACTER_ATTRIBUTE_STRIKETHROUGH) != 0
                        ? ":STRIKETHROUGH:"
                        : "")
    ...
    public void assertForegroundColorAt(int externalRow, int column, int color) {
        long style =
                mTerminal
                        .getScreen()
                        .mLines[mTerminal.getScreen().externalToInternalRow(externalRow)]
                        .getStyle(column);
