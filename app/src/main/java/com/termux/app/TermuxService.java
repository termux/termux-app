...
/**
 * A service holding a list of {@link TermuxSession} in {@link TermuxShellManager#mTermuxSessions}
 * and background {@link AppShell} in {@link TermuxShellManager#mTermuxTasks}, showing a foreground
 * notification while running so that it is not terminated. The user interacts with the session
 * through {@link TermuxActivity}, but this service may outlive the activity when the user or the
 ...
 *
 * <p>In order to keep both terminal sessions and spawned processes (who may outlive the terminal
 * sessions) alive as long as wanted by the user this service is a foreground service, {@link
 * Service#startForeground(int, Notification)}.
 *
 * <p>Optionally may hold a wake and a wifi lock, in which case that is shown in the notification -
 * see {@link #buildNotification()}.
 */
public final class TermuxService extends Service
        implements AppShell.AppShellClient, TermuxSession.TermuxSessionClient {
    ...
    /**
     * The full implementation of the {@link TerminalSessionClient} interface to be used by {@link
     * TerminalSession} that holds activity references for activity related functions. Note that the
     * service may often outlive the activity, so need to clear this reference.
     */
    private TermuxTerminalSessionActivityClient mTermuxTerminalSessionActivityClient;

    /**
     * The basic implementation of the {@link TerminalSessionClient} interface to be used by {@link
     * TerminalSession} that does not hold activity references and only a service reference.
     */
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        // If this service really do get killed, there is no point restarting it automatically - let
        // the user do on next
        // start of {@link Term):
     ...
     *
     * <p>For TermuxSessions, all sessions will be killed, whether user manually exited Termux or if
     ...
     *
     * <p>For TermuxTasks, only tasks that were started by a plugin which **expects** the result
     ...
     * running until the termux app process is killed by android, like by OOM, so we let them run as
     * long as they can.
     *
     * <p>Some plugin execution commands may not have been processed and added to mTermuxSessions
     * and mTermuxTasks lists before the service is killed, so we maintain a separate
     ...
     *
     * <p>Note that if user didn't manually exit Termux and if onDestroy() was directly called
     * because of unintended shutdown, like android deciding to kill the service, then there will be
     * no guarantee that onDestroy() will be allowed to finish and termux app process may be killed
     * before it has finished. This means that in those cases some results may not be sent back to
     * their creators for plugin commands but we still try to process whatever results can be
     * processed despite the unreliable behaviour of onDestroy().
     *
     * <p>Note that if don't kill the processes started by plugins which **expect** the result back
     * and notify their creators that they have been killed, then they may get stuck waiting for the
     * results forever like in case of commands started by Termux:Tasker or RUN_COMMAND intent,
     * since once TermuxService has been killed, no result will be sent back. They may still get
     * stuck if termux app process gets killed, so for this case reasonable timeout values should be
     * used, like in Tasker for the Termux:Tasker actions.
     *
     * <p>We make copies of each list since items are removed inside the loop.
     */
    private synchronized void killAllTermuxExecutionCommands() {
        ...
        List<ExecutionCommand> pendingPluginExecutionCommands =
                new ArrayList<>(mShellManager.mPendingPluginExecutionCommands);
    ...
    /**
     * Process {@link TERMUX_SERVICE#ACTION_SERVICE_EXECUTE} intent to execute a shell command in a
     * foreground TermuxSession or in a background TermuxTask.
     */
    private void actionServiceExecute(Intent intent) {
        ...
        ExecutionCommand executionCommand =
                new ExecutionCommand(TermuxShellManager.getNextShellId());

        executionCommand.executableUri = intent.getData();
        executionCommand.isPluginExecutionCommand = true;

        // If EXTRA_RUNNER is passed, use that, otherwise check EXTRA_BACKGROUND and default to
        // Runner.TERMINAL_SESSION
        executionCommand.runner =
                IntentUtils.getStringExtraIfSet(
                        intent,
        ...
        if (Runner.runnerOf(executionCommand.runner) == null) {
            ...
            TermuxPluginUtils.processPluginExecutionCommandError(
        ...
        if (executionCommand.executableUri != null) {
            Logger.logVerbose(
                    LOG_TAG,
                    "uri: \""
                            + executionCommand.executableUri
                            + "\", path: \""
            ...
            executionCommand.executable =
                    UriUtils.getUriFilePathWithFragment(executionCommand.executableUri);
            executionCommand.arguments =
                    IntentUtils.getStringArrayExtraIfSet(
                            intent, TERMUX_SERVICE.EXTRA_ARGUMENTS, null);
            ...
            executionCommand.backgroundCustomLogLevel =
                    IntentUtils.getIntegerExtraIfSet(
                            intent, TERMUX_SERVICE.EXTRA_BACKGROUND_CUSTOM_LOG_LEVEL, null);
        }
        ...
        executionCommand.shellCreateMode =
                IntentUtils.getStringExtraIfSet(
                        intent, TERMUX_SERVICE.EXTRA_SHELL_CREATE_MODE, null);
        executionCommand.commandLabel =
                IntentUtils.getStringExtraIfSet(
                        intent, TERMUX_SERVICE.EXTRA_COMMAND_LABEL, "Execution Intent Command");
        executionCommand.commandDescription =
                IntentUtils.getStringExtraIfSet(
                        intent, TERMUX_SERVICE.EXTRA_COMMAND_DESCRIPTION, null);
        ...
        executionCommand.resultConfig.resultDirectoryPath =
                IntentUtils.getStringExtraIfSet(
                        intent, TERMUX_SERVICE.EXTRA_RESULT_DIRECTORY, null);
        if (executionCommand.resultConfig.resultDirectoryPath != null) {
            ...
            executionCommand.resultConfig.resultFileBasename =
                    IntentUtils.getStringExtraIfSet(
                            intent, TERMUX_SERVICE.EXTRA_RESULT_FILE_BASENAME, null);
            executionCommand.resultConfig.resultFileOutputFormat =
                    IntentUtils.getStringExtraIfSet(
                            intent, TERMUX_SERVICE.EXTRA_RESULT_FILE_OUTPUT_FORMAT, null);
            executionCommand.resultConfig.resultFileErrorFormat =
                    IntentUtils.getStringExtraIfSet(
                            intent, TERMUX_SERVICE.EXTRA_RESULT_FILE_ERROR_FORMAT, null);
            executionCommand.resultConfig.resultFilesSuffix =
                    IntentUtils.getStringExtraIfSet(
                            intent, TERMUX_SERVICE.EXTRA_RESULT_FILES_SUFFIX, null);
        }
    ...
    private void executeTermuxTaskCommand(ExecutionCommand executionCommand) {
        ...
        if (ShellCreateMode.NO_SHELL_WITH_NAME.equals(shellCreateMode)) {
            ...
            if (newTermuxTask != null)
                Logger.logVerbose(
                        LOG_TAG,
                        "Existing TermuxTask with \""
                                + executionCommand.shellName
                                + "\" shell name found for shell create mode \""
                                + shellCreateMode.getMode()
                                + "\"");
            else
                Logger.logVerbose(
                        LOG_TAG,
                        "No existing TermuxTask with \""
                                + executionCommand.shellName
                                + "\" shell name found for shell create mode \""
                                + shellCreateMode.getMode()
                                + "\"");
    ...
    /** Create a TermuxTask. */
    @Nullable
    public AppShell createTermuxTask(
            String executablePath, String[] arguments, String stdin, String workingDirectory) {
    ...
    public synchronized AppShell createTermuxTask(ExecutionCommand executionCommand) {
        ...
        if (!Runner.APP_SHELL.equalsRunner(executionCommand.runner)) {
            Logger.logDebug(
                    LOG_TAG,
                    "Ignoring wrong runner \""
                            + executionCommand.runner
                            + "\" command passed to createTermuxTask()");
            return null;
        }
        ...
        AppShell newTermuxTask =
                AppShell.execute(
                        this, executionCommand, this, new TermuxShellEnvironment(), null, false);
        if (newTermuxTask == null) {
            Logger.logError(
                    LOG_TAG,
        ...
        mHandler.post(
                () -> {
                    if (termuxTask != null) {
                        ...
                        // If the execution command was started for a plugin, then process the
                        // results
    ...
    private void executeTermuxSessionCommand(ExecutionCommand executionCommand) {
        ...
        if (ShellCreateMode.NO_SHELL_WITH_NAME.equals(shellCreateMode)) {
            ...
            if (newTermuxSession != null)
                ...
                Logger.logVerbose(
                        LOG_TAG,
                        "No existing TermuxSession with \""
                                + executionCommand.shellName
                                + "\" shell name found for shell create mode \""
                                + shellCreateMode.getMode()
                                + "\"");
    ...
    /**
     * Create a {@link TermuxSession}. Currently called by {@link
     * TermuxTerminalSessionActivityClient#addNewSession(boolean, String)} to add a new {@link
     * TermuxSession}.
     */
    @Nullable
    ...
    public synchronized TermuxSession createTermuxSession(ExecutionCommand executionCommand) {
        ...
        // Otherwise if command was manually started by the user like by adding a new terminal
        // session,
        ...
        // No need to recreate the activity since it likely just started and theme should already
        // have applied
    ...
    public void onTermuxSessionExited(final TermuxSession termuxSession) {
        if (termuxSession != null) {
            ...
            // Notify {@link TermuxSessionsListViewController} that sessions list has been updated
            // if
            // activity in is foreground
            if (mTermuxTerminalSessionActivityClient != null)
                mTermuxTerminalSessionActivityClient.termuxSessionListNotifyUpdated();
            ...
            default:
                Logger.logError(
                        ...
                        "Invalid sessionAction: \""
                                + sessionAction
                                + "\". Force using default sessionAction.");
                handleSessionAction(
                        TERMUX_SERVICE
                                .VALUE_EXTRA_SESSION_ACTION_SWITCH_TO_NEW_SESSION_AND_OPEN_ACTIVITY,
                        newTerminalSession);
    ...
    private void startTermuxActivity() {
        // For android >= 10, apps require Display over other apps permission to start foreground
        // activities
        // from background (services). If it is not granted, then TermuxSessions that are started
        // will
    ...
    /**
     * If {@link TermuxActivity} has not bound to the {@link TermuxService} yet or is destroyed,
     * then interface functions requiring the activity should not be available to the terminal
     * sessions, so we just return the {@link #mTermuxTerminalSessionServiceClient}. Once {@link
     * TermuxActivity} bind callback is received, it should call {@link
     * #setTermuxTerminalSessionClient} to set the {@link
     * TermuxService#mTermuxTerminalSessionActivityClient} so that further terminal sessions are
     * directly passed the {@link TermuxTerminalSessionActivityClient} object which fully implements
     * the {@link TerminalSessionClient} interface.
     *
     * @return Returns the {@link TermuxTerminalSessionActivityClient} if {@link TermuxActivity} has
     ...
     * {@link TermuxService#mTermuxTerminalSessionActivityClient} variable and update the {@link
     * TerminalSession} and {@link TerminalEmulator} clients in case they were passed {@link
     * TermuxTerminalSessionServiceClient} earlier.
     *
     * @param termuxTerminalSessionActivityClient The {@link TermuxTerminalSessionActivityClient}
     *     object that fully implements the {@link TerminalSessionClient} interface.
     */
    ...
    /**
     * This should be called when {@link TermuxActivity} has been destroyed and in {@link
     * #onUnbind(Intent)} so that the {@link TermuxService} and {@link TerminalSession} and {@link
     * TerminalEmulator} clients do not hold an activity references.
     */
    ...
    private Notification buildNotification() {
        ...
        // If holding a wake or wifi lock consider the notification of high priority since it's
        // using power,
        ...
        Notification.Builder builder =
                NotificationUtils.geNotificationBuilder(
                        this,
        ...
        Intent exitIntent =
                new Intent(this, TermuxService.class).setAction(TERMUX_SERVICE.ACTION_STOP_SERVICE);
        builder.addAction(
        ...
        String newWakeAction =
                wakeLockHeld ? TERMUX_SERVICE.ACTION_WAKE_UNLOCK : TERMUX_SERVICE.ACTION_WAKE_LOCK;
        Intent toggleWakeLockIntent =
                new Intent(this, TermuxService.class).setAction(newWakeAction);
        String actionTitle =
                res.getString(
                        wakeLockHeld
    ...
    private synchronized void updateNotification() {
        if (mWakeLock == null
                ...
                && mShellManager.mTermuxTasks.isEmpty()) {
    ...
    @Nullable
    public synchronized TermuxSession getTermuxSessionForTerminalSession(
