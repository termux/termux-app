...
public class TermuxTerminalViewClient extends TermuxTerminalViewClientBase {
    ...
    /**
     * Keeping track of the special keys acting as Ctrl and Fn for the soft keyboard and other
     * hardware keys.
     */
    ...
    public TermuxTerminalViewClient(
    ...
    public void onCreate() {
        ...
        mActivity
                .getTerminalView()
                .setKeepScreenOn(mActivity.getPreferences().shouldKeepScreenOn());
    }

    /** Should be called when mActivity.onStart() is called */
    public void onStart() {
        // Set {@link TerminalView#TERMINAL_VIEW_KEY_LOGGING_ENABLED} value
        // Also required if user changed the preference from {@link TermuxSettings} activity and
        // returns
        ...
        // Piggyback on the terminal view key logging toggle for now, should add a separate toggle
        // in future
        mActivity
                .getTermuxActivityRootView()
    ...
    public void onResume() {
        ...
        if (mActivity.getTerminalView().mEmulator != null) {
            ...
            // If emulator is already set, then start blinker now, otherwise wait for
            // onEmulatorSet()
            // event to start it. This is needed since onEmulatorSet() may not be called after
            // TermuxActivity is started after device display timeout with double tap and not power
            // button.
    ...
    public void onEmulatorSet() {
        if (!mTerminalCursorBlinkerStateAlreadySet) {
            ...
            // TermuxActivity.onServiceConnected() and then the multiple calls to
            // TerminalView.updateSize()
            // where the final one eventually sets the mEmulator when width/height is not 0.
            // Otherwise
    ...
    public boolean onKeyUp(int keyCode, KeyEvent e) {
        // If emulator is not set, like if bootstrap installation failed and user dismissed the
        // error
    ...
    public boolean readExtraKeysSpecialButton(SpecialButton specialButton) {
        ...
        if (state == null) {
            Logger.logError(
                    LOG_TAG,
                    "Failed to read an unregistered "
                            + specialButton
                            + " special button value from extra keys.");
            return false;
        }
        return state;
    }
        ...
        if (mVirtualFnKeyDown) {
            ...
            switch (lowerCase) {
                ...
                case 'k':
                    ...
                    mVirtualFnKeyDown =
                            false; // force disable fn key down to restore keyboard input into
                                   // terminal view, fixes termux/termux-app#1420
                    break;
            }
    ...
    private void setSessionShortcuts() {
        ...
        // The {@link TermuxPropertyConstants#MAP_SESSION_SHORTCUTS} stores the session shortcut key
        // and action pair
        for (Map.Entry<String, Integer> entry :
                TermuxPropertyConstants.MAP_SESSION_SHORTCUTS.entrySet()) {
            // The mMap stores the code points for the session shortcuts while loading properties
            Integer codePoint =
                    (Integer)
                            mActivity
                                    .getProperties()
                                    .getInternalPropertyValue(entry.getKey(), true);
            // If codePoint is null, then session shortcut did not exist in properties or was
            // invalid
    ...
    public void onToggleSoftKeyboardRequest() {
        ...
        if (mActivity.getProperties().shouldEnableDisableSoftKeyboardOnToggle()) {
            ...
            if (!KeyboardUtils.areDisableSoftKeyboardFlagsSet(mActivity)) {
                ...
                // Show with a delay, otherwise pressing keyboard toggle won't show the keyboard
                // after
    ...
    public void setSoftKeyboardState(boolean isStartup, boolean isReloadTermuxProperties) {
        ...
        // If soft keyboard is disabled by user for Termux (check function docs for Termux behaviour
        // info)
        if (KeyboardUtils.shouldSoftKeyboardBeDisabled(
            ...
            // Set flag to automatically push up TerminalView when keyboard is opened instead of
            // showing over it
            ...
            if (isStartup && mActivity.getProperties().shouldSoftKeyboardBeHiddenOnStartup()) {
                ...
                // Required to keep keyboard hidden when Termux app is switched back from another
                // app
                ...
                .setOnFocusChangeListener(
                        new View.OnFocusChangeListener() {
                            ...
                            public void onFocusChange(View view, boolean hasFocus) {
                                // Force show soft keyboard if TerminalView or toolbar text input
                                // view has
                                // focus and close it if they don't
                                boolean textInputViewHasFocus = false;
                                final EditText textInputView =
                                ...
                                if (hasFocus || textInputViewHasFocus) {
                                    ...
                                    Logger.logVerbose(
                                            LOG_TAG, "Showing soft keyboard on focus change");
                                } else {
                                    Logger.logVerbose(
                                            LOG_TAG, "Hiding soft keyboard on focus change");
                                }
        ...
        // Do not force show soft keyboard if termux-reload-settings command was run with hardware
        // keyboard
        // or soft keyboard is to be hidden or is disabled
        if (!isReloadTermuxProperties && !noShowKeyboard) {
            // Request focus for TerminalView
            // Also show the keyboard, since onFocusChange will not be called if TerminalView
            // already
    ...
    public void setTerminalCursorBlinkerState(boolean start) {
        if (start) {
            ...
            else Logger.logError(LOG_TAG, "Failed to start cursor blinker");
        } else {
                        ...
                        .setItems(
                                ...
                                (di, which) -> {
                                    ...
                                    ShareUtils.copyTextToClipboard(
                                            ...
                                            url,
                                            mActivity.getString(
                                                    R.string.msg_select_url_copied_to_clipboard));
                                })
    ...
    public void reportIssueFromTranscript() {
        ...
        MessageDialogUtils.showMessage(
                ...
                mActivity.getString(R.string.action_no),
            ...
            public void run() {
                ...
                if (addTermuxDebugInfo) {
                    ...
                    reportString
                            ...
                            .append(
                                    TermuxUtils.getAppInfoMarkdownString(
                                            mActivity, TermuxUtils.AppInfoMode.TERMUX_PACKAGE));
                }
