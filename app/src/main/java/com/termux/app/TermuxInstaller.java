 ...
 *
 * <p>(1) If $PREFIX already exist, assume that it is correct and be done. Note that this relies on
 * that we do not create a broken $PREFIX directory below.
 *
 * <p>(2) A progress dialog is shown with "Installing..." message and a spinner.
 *
 * <p>(3) A staging directory, $STAGING_PREFIX, is cleared if left over from broken installation
 * below.
 *
 * <p>(4) The zip file is loaded from a shared library.
 *
 * <p>(5) The zip, containing entries relative to the $PREFIX, is is downloaded and extracted by a
 * zip input stream continuously encountering zip file entries:
 *
 * <p>(5.1) If the zip entry encountered is SYMLINKS.txt, go through it and remember all symlinks to
 * setup.
 *
 * <p>(5.2) For every other zip entry, extract it into $STAGING_PREFIX and set execute permissions
 * if necessary.
 */
final class TermuxInstaller {
    ...
    static void setupBootstrapIfNeeded(final Activity activity, final Runnable whenDone) {
        ...
        // This will also call Context.getFilesDir(), which should ensure that termux files
        // directory
        ...
        // If prefix directory exists, even if its a symlink to a valid directory and symlink is not
        // broken/dangling
        if (FileUtils.directoryFileExists(TERMUX_PREFIX_DIR_PATH, true)) {
            if (TermuxFileUtils.isTermuxPrefixDirectoryEmpty()) {
                Logger.logInfo(
                        LOG_TAG,
                        "The termux prefix directory \""
                                + TERMUX_PREFIX_DIR_PATH
                                + "\" exists but is empty or only contains specific unimportant"
                                + " files.");
            } else {
                whenDone.run();
                return;
                ...
                try {
                    ...
                    if (error != null) {
                        showBootstrapErrorDialog(
                                activity, whenDone, Error.getErrorMarkdownString(error));
                        return;
                    }
                    ...
                    error =
                            FileUtils.deleteFile(
                                    "termux prefix directory", TERMUX_PREFIX_DIR_PATH, true);
                    if (error != null) {
                        showBootstrapErrorDialog(
                                activity, whenDone, Error.getErrorMarkdownString(error));
                        return;
                    ...
                    try (ZipInputStream zipInput =
                        ...
                        while ((zipEntry = zipInput.getNextEntry()) != null) {
                            if (zipEntry.getName().equals("SYMLINKS.txt")) {
                                ...
                                File targetFile =
                                        new File(TERMUX_STAGING_PREFIX_DIR_PATH, zipEntryName);
                    ...
                    showBootstrapErrorDialog(
                            ...
                            Logger.getStackTracesMarkdownString(
                                    null, Logger.getStackTracesStringArray(e)));
    ...
    public static void showBootstrapErrorDialog(
            Activity activity, Runnable whenDone, String message) {
        ...
        new Thread() {
            public void run() {
                try {
                    ...
                    Logger.logInfo(
                            LOG_TAG,
                            "Setting up storage symlinks at ~/storage/shared, ~/storage/downloads,"
                                + " ~/storage/dcim, ~/storage/pictures, ~/storage/music and"
                                + " ~/storage/movies for directories in \""
                    ...
                    if (dirs != null && dirs.length > 0) {
                        for (int i = 0; i < dirs.length; i++) {
                            ...
                            Logger.logInfo(
                                    LOG_TAG,
                                    "Setting up storage symlinks at ~/storage/"
                                            + symlinkName
                                            + " for \""
                                            + dir.getAbsolutePath()
                                            + "\".");
                    ...
                    Logger.logStackTraceWithMessage(
                            LOG_TAG, "Setup Storage Error: Error setting up link", e);
                    TermuxCrashUtils.sendCrashReportNotification(
                            context,
                            LOG_TAG,
                            title,
